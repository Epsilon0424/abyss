...
ARTIFACTS = {
    ...
    "꿈의 저편으로": {
        ...
        "unique_buffs": {"atk_pct": 0.15},
        # 밀키웨이 전용 아티팩트 효과
        # - 미지의 영역: 밀키웨이 본인과 주변 아군 모든 속성 피해 +20% (30초)
        # - 뜻밖의 선로: 적의 신비 속성 내성 -12% (30초)
        "milky_way": {
            "party_all_elem_dmg": 0.20,
            "mystic_res_reduction": 0.12,
        },
    },
    ...
    "외딴 별의 여정": {
        "base_stats": {"atk_pct": 0.35},
        "unique_stats": {},
        # 발동 구간이 서로 달라 스타더스트 사이클 함수에서 개별 적용한다.
        "unique_buffs": {},
        "stardust": {
            "shadow_dmg_increase": 1.00,
            "star_path_crit_dmg": 0.75,
            "cosmos_basic_dmg": 0.50,
        },
    },

    "보물의 방향": {
        # Where the Compass Points / 잭프루트맛 쿠키 전용 아티팩트
        "base_stats": {"crit_rate": 0.25},
        "unique_stats": {},
        # 순풍/뱃노래는 반복 사이클에서 30초 버프가 계속 갱신되어 상시 유지로 계산한다.
        "unique_buffs": {"crit_dmg": 0.60, "crit_rate": 0.25, "all_elem_dmg": 0.60},
        "jackfruit": {
            "tailwind_duration": 30.0,
            "shanty_duration": 30.0,
            "link_cooldown_reset_on_ult": True,
        },
    },

    "꿈결 같은 휴식": {
        # Dreamlike Breaktime / 스테인드누가맛 쿠키 전용 아티팩트
        "base_stats": {"atk_pct": 0.35},
        "unique_stats": {},
        # 전투 시작 시 본인 공격력 +10%
        "unique_buffs": {"atk_pct": 0.10},
        # [얼룩진 기억] 적중 시 [쏟아지는 기억]: 적이 받는 기본 공격 피해 +35% (20초)
        # 실제 적용은 cookie/stained_nougat.py 및 cookie/common.py에서 처리한다.
        "stained_nougat": {"enemy_basic_taken_inc": 0.35, "duration": 20.0},
    },
