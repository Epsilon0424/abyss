...
# 공격력 표기: 941 + 호감도 38
# 승급 공격력 +30%: 호감도 제외 기본공격력 941 기준 역산
...
# 이벤트 구성: 종류·계수·독·흑보리 보너스·다음 8타 보너스
# 이벤트 종류: 기본·특수·궁극기·특수 발동·기본 발동
# =====================================================
# 고속 이벤트 전처리
# =====================================================
def _bb_precompute_fast_events() -> Tuple[List[Tuple[str, float, bool, bool, bool]], float]:
    ...
    for tok in BB_CYCLE_TOKENS:
        # 탄창 규칙: 기본 공격 잔탄 존재 시 강화 공격 치환·탄약 소비
        action = tok
        if tok == "B" and ammo > 0:
            action = "E"
            ammo -= 1
        ...
        if tok == "S":
            ...
            if special_use_count % 2 == 1:
                # 사냥전술 1회차: 블랙 스플래시 포션만 적중하고 사냥꾼의 독을 부여
...
def black_barley_cycle_damage_fast(stats: Dict[str, float], party: List[str], artifact_name: str) -> Dict[str, float]:
    ...
    # 파티 공용 사냥꾼의 독 중복 제외
    # 메인 흑보리 단독일 때만 본인 사이클의 독 적용 타이밍에 +10%를 추가
