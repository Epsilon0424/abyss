...
def black_barley_cycle_damage_fast(stats: Dict[str, float], party: List[str], artifact_name: str) -> Dict[str, float]:
    ...
    promo_basic_mult = float(base_st.get("promo_basic_dmg_mult", 1.0))
    promo_special_mult = float(base_st.get("promo_special_dmg_mult", 1.0))
    promo_ult_mult = float(base_st.get("promo_ult_dmg_mult", 1.0))
    ...
    for kind, coeff, use_poison_unit, use_black_bonus, use_next8_bonus in _BB_FAST_EVENTS:
        ...
        if kind == "basic":
            ...
            dmg = skill_damage_from_start(
                ...
                float(coeff),
                "basic",
                extra_skill_mult=extra_mult * promo_basic_mult,
            )
            total_direct += dmg
            breakdown["basic"] += dmg
            ...
            dmg = skill_damage_from_start(
                ...
                float(coeff),
                "special",
                extra_skill_mult=promo_special_mult,
            )
            total_direct += dmg
            ...
            if kind == "proc_special":
                ...
                dmg = skill_damage_from_start(
                    ...
                    float(coeff),
                    "basic",
                    extra_skill_mult=promo_basic_mult,
                )
