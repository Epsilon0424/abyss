...
# [노블레스 오블리주] 426% 추가 피해가 0/10/20/30% 증가
# 3차징: 8타 각각 426% × 1.20
# 4차징: 16타 각각 426% × 1.30
...
# 특수스킬은 보호막 스킬이라 피해 계산에서는 제외
# 실제 30초 사이클:
# 특 → 궁(인퓨전 진입) → 4차징 → 2평 → 4차징 → 특 →
# 4평 → 3차징 → 3차징 → 3평 → 3차징
#
# [퍼펙트 티 파티] 3차징은 기본 공격 피해,
# [애프터 디너 티] 4차징은 궁극기 피해로 분류
...
def blue_mallow_cycle_damage_fast(stats: Dict[str, float], party: List[str], artifact_name: str) -> Dict[str, float]:
    ...
    for tok in BLUE_MALLOW_CYCLE_TOKENS:
        ...
        if tok == "B":
            ...
            # 티 브레이크는 보호막 스킬이라 직접 피해는 없음
            breakdown["special"] += 0.0

        elif tok == "U":
            # 궁극기 버튼 인퓨전 진입·후속 4차징 피해
            dmg = 0.0
...
def optimize_blue_mallow_cycle(
    ...
    potentials = [dict(potential_override)] if potential_override is not None else blue_mallow_generate_potentials_common()
