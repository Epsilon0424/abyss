...
# Imports
# =====================================================
from typing import Any, Dict, List, Tuple, Optional, Callable, Union, Iterator
import math
...
def apply_seaz_passive(
    ...
    *,
    ...
    # - 그 외 쿠키: 기존 안내/계산 기준대로 공격력 12.5% / 모든 속성 피해 15% 적용
    # 허브그린드는 원래 12.5%/15%라 그대로이고, 플럼나이트만 해당 값으로 보정된다.
...
def apply_unique(
    ...
    *,
    ...
    # (2) 버프증폭 기본옵션
    # - 로드 나이트메어의 기억: 공격력 +15%, 버프증폭 +36%는 파티 버프 계산에 반영
    ...
    if ut == "striker_all_elem_support":
        # 별빛 질주: 모든 속성 피해 +15% (속성 강타가 반복되어 유지되는 것으로 적용)
...
def apply_party_buffs(
    ...
    def _buff_amp_for_owner(cookie_name: str) -> float:
        ...
        if cookie_name == "이슬맛 쿠키":
            ...
            try:
                ...
                moon_unique_raw = ""
            moon_unique = "NONE" if moon_unique_raw == "NONE" else (_selected_party_unique_name(cookie_name) or "달빛술사 쿠키의 기억")
    ...
    # =====================================================
    # [DPS PARTY DEBUFF] 보스에 묻는 공용 '쿠키에게 받는 피해 증가'
    # - 스타더스트 [별무리] 10중첩: 쿠키에게 받는 피해 +10%
    # - 흑보리 [사냥꾼의 독]: 쿠키에게 받는 피해 +10%
    # 같은 쿠키가 2명 있어도 동일 디버프는 1회만 적용한다.
    # 메인 쿠키가 스타더스트/흑보리인 경우에도 다른 파티원의 딜 계산에서는
    # 해당 메인 쿠키가 party 목록에 들어오므로 팀 전체에 공유된다.
    # =====================================================
    ...
    def _apply_milky_way_party_effects():
        ...
        if COOKIE_ELEMENT.get(main_cookie_name, "") == "mystic":
            # 뜻밖의 선로는 아티팩트 고정 효과이므로 디버프 증폭 없이 적용한다.
    ...
    def _apply_moonlight_party_effects():
        ...
        # 보호막은 딜 계산에는 직접 반영하지 않고, 유니크 설탕유리조각(멜랑크림 쿠키의 순수한 기억)의
        # 방어 관통 유지 조건으로만 본다.
        if in_party_moonlight and main_cookie_name != "달빛술사 쿠키":
    ...
    # 현재 메인 쿠키의 실제 증폭값은 본인 스냅샷으로 되돌린다.
    ...
    # 한 번씩만 합산한 값으로 정규화한다.
    # 기존 party_*_total은 파티 유니크를 적용하는 과정에서 이미 가정값에 포함된
    # 달빛술사 유니크 증폭이 다시 더해져 36%가 중복 표시될 수 있었다.
...
def _moonlight_base_debuff_amp_without_potential(
    ...
    da += _moonlight_unique_debuff_amp(unique_name or "달빛술사 쿠키의 기억")
    return max(0.0, da)


def moonlight_fixed_debuff_slots_for_equip(equip_name: str) -> int:
    """달빛술사 장비별 고정 디버프 증폭 잠재력 칸 수."""
...
def moonlight_auto_potentials_for_combo(
    ...
    """파티 기여 계산 등에 쓰는 달빛술사 잠재력 기본 형태.

    디버프 증폭 잠재력은 장비에 따라 고정한다.
    - 유성우의 향연: 1칸
    - 시간관리국의 제복: 4칸

    메인 달빛술사의 실제 계산에서는 남은 칸을 별도 잠재력 후보로 생성해
    DPS가 가장 높은 조합을 선택한다. 이 함수는 파티 기여 계산에서
    고정 디버프 증폭 칸 수를 일관되게 참조하기 위한 기본값만 반환한다.
    """
...
def _assumed_moonlight_debuff_amp_for_party(
    ...
    장비에 따라 디버프 증폭 잠재력을 고정한다.
    유성우의 향연은 1칸, 시간관리국의 제복은 4칸을 사용한다.
    메인 달빛술사의 남은 잠재력은 DPS 기준으로 별도 최적화한다.
    """
...
def _assumed_milky_way_debuff_amp_for_party() -> float:
    """밀키웨이 장비 유래 디버프 증폭은 선택 장비 기준으로 apply_party_buffs에서 반영한다."""
    return 0.0
...
def _apply_party_amp_totals(stats: Dict[str, float], party: List[str], main_cookie_name: str) -> None:
    ...
    if "달빛술사 쿠키" in (party or []) and main_cookie_name != "달빛술사 쿠키":
        ...
        if (not moon_unique_for_ba) or moon_unique_for_ba.upper() == "AUTO":
            moon_unique_for_ba = "달빛술사 쿠키의 기억"
        ba += _assumed_moonlight_buff_amp_for_party(
...
def build_stats_for_combo(
    ...
    # - 일반 "기본 공격 피해 증가 / 특수 스킬 피해 증가 / 궁극기 피해 증가"는
    #   다른 동일 스탯과 합연산되도록 해당 스탯 축에 직접 더한다.
    # =====================================================
...
def _damage_stats_with_shards(
    ...
    """고정된 장비 조합의 템플릿에 일반 조각 배분만 적용한다."""
...
def optimize_damage_shards_for_fixed_combo(
    ...
    *,
    ...
    """장비별 정밀 전수조사 전에 사용할 빠른 1차 조각 후보를 계산한다.

    치명타 확률에 필요한 최소 칸을 먼저 고정한 뒤, 남은 칸을 한 칸씩 실제
    사이클 DPS가 가장 높은 공격 옵션에 배정하고 한 칸 교환으로 보정한다.
    최종 결과는 이 함수로 선별된 장비별 상위 후보를
    ``optimize_damage_shards_top4_for_fixed_combo``에서 다시 계산해 확정한다.
    """
...
def _select_top_damage_shard_keys(
    ...
    *,
    ...
    """고정 조합에서 조각 1칸당 실제 DPS 증가량이 높은 공격 옵션을 고른다."""
    active_keys = tuple(
        key for key in damage_keys
        if key in SHARD_INC and key != "crit_rate" and float(SHARD_INC.get(key, 0.0)) != 0.0
    )
...
def _iter_exact_shard_allocations(
    ...
    """선택된 조각 옵션에 total_slots를 1칸 단위로 모두 배분한다."""
...
def optimize_damage_shards_top4_for_fixed_combo(
    ...
    *,
    ...
    """상위 공격 옵션을 step=1로 전수조사하고 전체 옵션 교환으로 보정한다.

    장비·유니크·잠재력이 고정된 템플릿에서 치확 100%에 필요한 조각을
    먼저 고정한다. 남은 칸은 1칸 DPS 효율이 높은 상위 옵션에 대해 모든
    배분을 전수조사한다. 이후 전수조사 대상에 들지 않은 공격 옵션까지
    포함해 한 칸 교환을 반복하여 결과를 보정한다.
    """
    required_crit_slots = int(required_crit_slots)
    if required_crit_slots < 0 or required_crit_slots > NORMAL_SLOTS:
        raise ValueError("required_crit_slots는 0~41 범위여야 합니다.")
...
def _effective_crit_rate_with_promo(stats: Dict[str, float]) -> float:
    """승급 배율과 별도 치확 버프를 포함한 실제 치명타 확률의 클램프 전 값."""
...
def _min_crit_slots_needed_for_crit100_generic(template: Dict[str, float]) -> Optional[int]:
    """치확 100%에 필요한 설탕유리조각의 최소 칸 수를 계산한다."""
...
def generate_damage_potential_candidates(
    *,
    ...
    고정 잠재력을 제외한 남은 칸을 ``free_keys``에 지정된 공격 옵션에
    배분한다. 기본값은 기존과 동일하게 치명타 확률, 공격력 %,
    치명타 피해, 방어력 관통만 탐색한다. 특정 쿠키에서 속성 공격력까지
    자유 탐색해야 할 때만 ``free_keys``에 ``elem_atk``를 추가한다.

    잠재력 장착 제한도 여기서 함께 보장한다.
    - 속성 공격력: 최대 2칸
    - 버프 증폭 / 디버프 증폭: 각각 최대 4칸
    - 방어력 관통: 기존 규칙대로 ``armor_pen_cap``(기본 4칸)
    """
    fixed_stats = {
        "debuff_amp": 0,
        "crit_rate": 0,
    ...
    # 게임 내 잠재력 장착 개수 제한. 고정값 단계부터 잘못된 후보를 막는다.
    fixed_caps = {
        "elem_atk": 2,
        "buff_amp": 4,
        "debuff_amp": 4,
...
def should_evaluate_conditional_crit_potential(
    ...
    *,
    ...
    """현재 잠재력 후보를 치확 100% 조합 비교에 사용할 수 있는지 확인한다.

    잠재력 치확을 0칸 또는 특정 고정 칸으로 제한하지 않는다. 잠재력에
    배정된 치확을 반영한 상태에서 필요한 최소 설탕유리조각 치확 칸 수를
    계산할 수 있으면 후보로 유지한다. 따라서 잠재력 치확과 설탕유리조각
    치확을 섞은 모든 가능한 조합이 실제 DPS 비교에 들어간다.

    ``legacy_crit_slots``와 ``eps``는 기존 호출부 호환을 위해 유지한다.
    """
