...
ISLE_FIXED_UNIQUE   = "멜랑크림 쿠키의 순수한 기억"
...
def isle_cycle_damage(stats: Dict[str, float], party: List[str], artifact_name: str) -> Dict[str, float]:
    ...
    base_for_sim.pop("_damage_context_cache", None)
    if promo_on:
    ...
    for tok in ISLE_CYCLE_TOKENS:
        ...
        if tok == "C":
            ...
            # 그림자/패시브 추가타는 기존 조건을 유지
            # 스택 3개·내부 재사용 대기시간 충족 시 발동
...
def optimize_isle_cycle(
    ...
    pot           = dict(potential_override) if potential_override is not None else dict(ISLE_FIXED_POT)
