...
# 만렙 표기 공격력 845에는 호감도 공격력 57이 포함
# 잭프루트 돌파: 치명타 확률, 공격력% 역산 제외
...
BASE_STATS_JACKFRUIT = {
    "잭프루트맛 쿠키": {
        ...
        "elem_atk": 0.0,
        # 2성 승급 공격력 +15%는 표기 845를 역산하지 않고 별도 공격력%로 적용
...
# 사용자 제공 30초 반복 사이클
# 이벤트 구성: 행동·2성 대지 피해·4성 기본공격 강화
# 2성 대지 피해 버프 연계 직후 특수·강화 특수 적용
...
def jackfruit_cycle_damage_fast(stats: Dict[str, float], party: List[str], artifact_name: str) -> Dict[str, float]:
    del artifact_name  # 고정 아티팩트 효과: 공통 스탯 빌드에서 반영
