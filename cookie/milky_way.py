...
# 만렙 표기 공격력 786·승급 공격력 20%·호감도 공격력 48 포함
# 기본 공격력 역산: 표기 786에서 호감도 공격력 48 선제외
MILKY_WAY_DISPLAY_ATK_WITH_FRIENDSHIP = 786.0
MILKY_WAY_PROMOTED_ATK_WITHOUT_FRIENDSHIP = (
    MILKY_WAY_DISPLAY_ATK_WITH_FRIENDSHIP - friendship_atk_for("밀키웨이맛 쿠키")
)
BASE_STATS_MILKY_WAY = {
    "밀키웨이맛 쿠키": {
        "atk": atk_from_promoted_base_without_friendship(
            MILKY_WAY_PROMOTED_ATK_WITHOUT_FRIENDSHIP,
            MILKY_WAY_PROMO_ATK_PCT,
        ),
...
def optimize_milky_way_cycle(
    ...
    potentials = [dict(potential_override)] if potential_override is not None else milky_way_allowed_potentials()
