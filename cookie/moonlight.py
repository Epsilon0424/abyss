...
# - 나머지 칸은 치확/공격력%/속성공격력/치피/방관 중 실제 DPS가
#   가장 높은 조합을 탐색한다.
...
def moonlight_allowed_equips() -> List[str]:
    ...
    # 유성우는 디벞증 잠재 1칸, 시간셋은 4칸을 고정하고 나머지는 DPS로 탐색한다.
    # 기본값은 유성우의 향연이다.
...
def moonlight_allowed_potentials_for_equip(equip_name: str) -> List[Dict[str, int]]:
    """장비별 디벞증 고정 칸을 제외한 나머지 잠재력을 DPS 후보로 생성한다."""
...
def _moonlight_greedy_shards_for_screening(
    ...
    """잠재력 후보 선별용 빠른 조각 최적화.

    41칸을 한 칸씩 실제 DPS가 가장 많이 오르는 조각에 배정한다.
    최종 후보는 아래 정밀 전수조사에서 다시 계산하므로 선별 단계에서만 사용한다.
    """
...
def _moonlight_exact_shards_for_template(
    ...
    """고정 장비/유니크/잠재력에서 기존 달술 조각 정밀 탐색을 수행한다."""
...
def optimize_moonlight_cycle(
    ...
    """장비별 디벞증 고정 칸을 유지하고 남은 잠재력/조각을 DPS로 최적화한다."""
    ...
    # 2차: 각 장비/유니크 조합의 상위 잠재력만 기존 1칸 정밀 조각 탐색으로 확정한다.
