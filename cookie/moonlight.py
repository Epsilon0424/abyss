...
def moonlight_count_shard_candidates(step: int = 1) -> int:
    """달빛술사 조각 후보 수를 실제 후보 생성 없이 계산한다."""
    steps = _moonlight_shard_steps(step)
    axes_count = 5

    if max(1, int(step or 1)) == 1:
        # 5개 탐색축과 남은 속성 공격력 축의 음이 아닌 정수 분할 수
...
def moonlight_generate_shard_candidates(step: int = 1) -> Iterator[Dict[str, int]]:
    """달빛술사 일반 설탕유리조각 후보를 메모리에 쌓지 않고 순차 생성한다.

    탐색 축은 치명타 피해, 모든 속성 피해, 공격력 %, 기본 공격 피해,
    궁극기 피해이며 특수 스킬 피해는 탐색하지 않는다. 남는 칸은
    속성 공격력으로 배정한다.
    """
    steps = _moonlight_shard_steps(step)
    axes = ["crit_dmg", "all_elem_dmg", "atk_pct", "basic_dmg", "ult_dmg"]
...
def optimize_moonlight_cycle(
    ...
    shard_candidate_count = moonlight_count_shard_candidates(step=step)
    ...
    for equip_name in equips:
        for unique_name in uniques:
            ...
            for sh in moonlight_generate_shard_candidates(step=step):
                done += 1
                if (done % tick) == 0:
                    emit(done / total)
