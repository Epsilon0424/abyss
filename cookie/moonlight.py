...
def moonlight_count_shard_candidates(
    step: int = 1,
    key_count: int = MOONLIGHT_ACTIVE_SHARD_KEY_COUNT,
) -> int:
    """선택된 조각 축에 41칸을 배분할 때의 후보 수를 계산한다."""
    key_count = max(1, int(key_count))
    axes_count = max(0, key_count - 1)
    step = max(1, int(step or 1))

    if step == 1:
        return math.comb(NORMAL_SLOTS + axes_count, axes_count)

    steps = _moonlight_shard_steps(step)

    def count_rec(idx: int, used: int) -> int:
        if idx >= axes_count:
            return 1
        total = 0
        for value in steps:
            if used + value > NORMAL_SLOTS:
                continue
            total += count_rec(idx + 1, used + value)
        return total

    return count_rec(0, 0)
...
def moonlight_generate_shard_candidates(
    ...
    """현재 장비 조건에서 선택된 조각 축에 41칸을 순차 배분한다."""
...
def _select_moonlight_shard_keys(
    ...
    *,
    ...
    """장비가 고정된 상태에서 1칸당 실제 DPS 증가량이 높은 축을 고른다."""
...
def _refine_moonlight_shards(
    ...
    """모든 공격 조각 사이의 1칸 교환을 반복해 장비별 결과를 보정한다."""
...
def optimize_moonlight_cycle(
    ...
    shard_candidate_count = moonlight_count_shard_candidates(
        step=step,
        key_count=MOONLIGHT_ACTIVE_SHARD_KEY_COUNT,
    )
    total = max(1, len(equips) * len(uniques) * shard_candidate_count)
    ...
    for equip_name in equips:
        for unique_name in uniques:
            ...
            if not is_valid_by_caps(template):
                done += shard_candidate_count
                continue

            # 장비/유니크/잠재력 조건을 고정한 뒤 모든 공격 조각의 1칸 DPS를
            # 비교해 1차 탐색축을 고른다. 장비가 바뀌면 이 목록도 다시 계산된다.
            active_keys = _select_moonlight_shard_keys(
                template,
                party,
                artifact_name,
            )
            template_best: Optional[dict] = None

            for sh in moonlight_generate_shard_candidates(
                step=step,
                shard_keys=active_keys,
            ):
                done += 1
                if (done % tick) == 0:
                    emit(done / total)
                ...
                if template_best is not None and float(cycle["dps"]) <= float(template_best["dps"]):
                    continue
            ...
            if template_best is None:
                continue

            # 1차 후보에 들지 않은 옵션도 한 칸 교환으로 다시 검사한다.
            refined_shards, refined_stats, refined_cycle = _refine_moonlight_shards(
                template,
                template_best["shards"],
                party,
                artifact_name,
            )
            template_best["shards"] = refined_shards
            template_best["stats"] = refined_stats
            template_best["dps"] = float(refined_cycle["dps"])
            template_best["cycle_total_damage"] = float(refined_cycle["total_damage"])
            template_best["cycle_total_time"] = float(refined_cycle.get("total_time", MOONLIGHT_CYCLE_TIME))
            template_best["cycle_breakdown"] = refined_cycle
            template_best["support_detail"] = moonlight_calc_support_metrics(refined_stats)
            template_best["buff_amp_total"] = refined_stats.get(
                "buff_amp_total", refined_stats.get("buff_amp", 0.0)
            )
            template_best["debuff_amp_total"] = refined_stats.get(
                "debuff_amp_total", refined_stats.get("debuff_amp", 0.0)
            )

            equip_best = best_by_equip.get(equip_name)
            if equip_best is None or float(template_best["dps"]) > float(equip_best.get("dps", 0.0)):
                best_by_equip[equip_name] = template_best

    for equip_name in equips:
        cur = best_by_equip.get(equip_name)
        if cur is not None and (best is None or float(cur["dps"]) > float(best.get("dps", 0.0))):
            best = cur
