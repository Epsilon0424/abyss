...
def _shining_precompute_fast_events() -> Tuple[List[Tuple[str, float]], float]:
    ...
    for tok in SHINING_CYCLE_TOKENS:
        if tok == "S":
            ...
            # 베리샤인 익스텐드 3타 이후부터 이후 모든 피해 50% 증가
            for i in range(SHINING_ULT_HITS):
                kind = "ult_hold_start" if i == 3 else "ult"
                events.append((kind, float(SHINING_ULT_HIT_COEFF)))
        elif tok == "B4":
            events.append(("basic", float(SHINING_BASIC_COEFF)))
        elif tok == "D":
            dash_count += 1
            if dash_count % 2 == 0:
                # 대시 2개가 한 짝이므로, 두 번째 대시에만 [베리 스로우] 추가타를 계산한다.
...
def shining_berry_cycle_damage_fast(stats: Dict[str, float], party: List[str]) -> Dict[str, float]:
    ...
    berryshine_hold_active = False

    for kind, coeff in _SHINING_FAST_EVENTS:
        if kind == "ult_hold_start":
            berryshine_hold_active = True
            kind = "ult"

        hold_mult = SHINING_BERRY_PROMO_ULT_HOLD_MULT if (SHINING_BERRY_PROMO_ENABLED and berryshine_hold_active) else 1.0

        if kind == "basic":
            dmg = skill_damage_from_start(stats, float(coeff), "basic", extra_skill_mult=hold_mult)
            breakdown["basic"] += dmg
        elif kind == "special":
            dmg = skill_damage_from_start(stats, float(coeff), "special", extra_skill_mult=special_extra_mult * hold_mult)
