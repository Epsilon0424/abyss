...
def _shining_precompute_fast_events() -> Tuple[List[Tuple[str, float]], float]:
    ...
    for tok in SHINING_CYCLE_TOKENS:
        if tok == "S":
            # 일반 사이클에서는 특수 스킬을 연속 입력하지 않고 첫 찌르기만 사용한다.
            events.append(("special", float(SHINING_SPECIAL_STAB_COEFF)))
            # 찌르기 카운터 적중으로 발생하는 샤이닝 스피어는 별도 궁극기 피해로 유지한다.
            ...
            for i in range(SHINING_ULT_HITS):
                # 앞 4타는 일반 궁극기 피해, 이후 26타에만 3초 유지 피해 +50%를 적용한다.
            ...
            dash_count += 1
            if dash_count % 2 == 0:
                # [또 다른 나]는 기본 공격만 궁극기 피해로 전환한다.
                # 대시 파생 공격인 [베리 스로우]는 궁극기 이후에도 기본 공격 피해로 계산한다.
