...
# Constants
# =====================================================
STAINED_NOUGAT_PROMO_ENABLED = True
STAINED_NOUGAT_FORCE_CRIT_100 = True
STAINED_NOUGAT_WEAPON_ATK_PCT = 0.52
STAINED_NOUGAT_WEAPON_FINAL_DMG = 0.30

STAINED_NOUGAT_PROMO_CRIT_RATE_MULT = 1.0
STAINED_NOUGAT_PROMO_BASE_ATK_MULT = 1.0
STAINED_NOUGAT_PROMO_DEF_PCT_MULT = 1.0
STAINED_NOUGAT_PROMO_HP_PCT_MULT = 1.0
...
# 공격력 표기: 842 + 호감도 51
# 승급 공격력 +30%는 호감도 제외 기본공격력(842)에만 역산하고, 호감도공은 최종 공격력 계산 마지막에 더한다.
...
# 기본공격 1세트(4평) = 1타 142% + 2타 142% + 3타 177.5% + 4타 106.5% × 3
...
# 패시브 [얼룩진 기억] = 390.5% × 6회 (5초 쿨타임 기준 30초 사이클 최대 6회 가정)
...
# [정화] : 쿠키에게 받는 피해 33.6% 증가 + 대지 속성 쿠키에게 받는 피해 20% 증가
...
def stained_nougat_generate_shard_candidates_no_cr(step: int = 3) -> List[Dict[str, int]]:
    step = max(1, int(step))
    total_slots = NORMAL_SLOTS
    keys = ["crit_dmg", "all_elem_dmg", "atk_pct", "basic_dmg", "passive_dmg"]
    out: List[Dict[str, int]] = []

    def dfs(i: int, left: int, cur: Dict[str, int]) -> None:
        if i == len(keys) - 1:
            cur[keys[i]] = left
            out.append(cur.copy())
            return
        for v in range(0, left + 1, step):
            cur[keys[i]] = v
            dfs(i + 1, left - v, cur)
