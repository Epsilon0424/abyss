...
# 표기 기본공격력 897에는 돌파 공격력 +30%가 포함
# 호감도 공격력 54는 최종 공격력 공식 마지막에 별도로 더
BASE_STATS_STARDUST = {
    "스타더스트 쿠키": {
...
# 기본공격 순서: 별의 그림자와 무관하게 1타 → 2타 → 3타 유지
...
# 치명타 확률은 잠재력과 조각을 섞어 100%가 되는 최소 칸을 먼저 배정
...
def optimize_stardust_cycle(
    ...
    del step  # 장비별 실제 피해량 기준 1칸 단위 배분
