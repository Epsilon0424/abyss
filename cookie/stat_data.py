...
def _apply_sugar_glass_one(stats: Dict[str, float], cookie_name_kr: str, *, is_owner: bool = False) -> None:
    ...
    if role == "strike":
        ...
        if crush >= 21:
            ...
            v = 0.0
        if v:
            stats["elem_res_reduction_raw"] = float(stats.get("elem_res_reduction_raw", 0.0)) + v
    ...
    if role == "support":
        ...
        if mark in mark_map:
            stats["elem_res_reduction_raw"] = float(stats.get("elem_res_reduction_raw", 0.0)) + mark_map[mark]
