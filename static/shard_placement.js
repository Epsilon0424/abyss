...
`;
    ...
    function cookieSimUniqueCardDistance(left, right, featureSize = 24) {
        ...
        // Card detection can move by one or two pixels between screenshots. Compare
        // the fixed-position thumbnails with a small translation allowance instead of
        // cropping each foreground component to a different bounding box.
        let bestDistance = Number.POSITIVE_INFINITY;
    ...
    function cookieSimExtractUniqueSpatialFeature(iconRoi, featureSize = 20) {
        ...
        for (let y = 0; y < iconRoi.rows; y++) {
            for (let x = 0; x < iconRoi.cols; x++) {
                ...
                // Keep the complete artwork, including its pale border.  Only the card's
                // gold field, the equipped tag, and edge remnants are removed.
        ...
        // Very wide, unequipped thumbnails can connect to the inner crop edge after
        // JPEG scaling.  In that case the edge filter may reject every contour, so
        // fall back to the already-cleaned foreground mask instead of losing the
        // artwork fingerprint entirely.
    ...
    async function cookieSimInitializeUniqueReferenceSamples() {
        ...
        cookieSimUniqueReferenceInitPromise = (async () => {
            for (const meta of Object.values(COOKIE_SIM_UNIQUE_REFERENCE_META)) {
                ...
                // Replace the frozen Python fallback only after every browser sample
                // has been generated. Uploaded cards and reference cards now use the
                // exact same OpenCV.js resize, masking, and pixel arithmetic.
    ...
    function cookieSimRecognizeUniqueShard(shape, visualFeature, artFeature, spatialFeature, cardFeature) {
        ...
        // The normal Moonlight memory has no supplied in-game thumbnail yet. When its
        // distinctive silhouette is read correctly, keep that result instead of forcing
        // it into one of the sampled classes.
        ...
        // First compare the complete normalized in-game card thumbnail. Uploaded
        // screenshots are cropped to the card's 6% inner area, and stored samples are
        // already saved from that same area. Keeping both in one coordinate system
        // prevents equipped tags from shifting the artwork comparison.
        if (Array.isArray(cardFeature) && cardFeature.length > 0) {
            ...
            if (cardMatches.length > 0) {
                ...
                // Supplied screenshots produce same-name distances below 0.12 and
                // different-name distances above 0.26. Leave margin for browser image
                // scaling while refusing to force an unknown unsampled unique shard.
        ...
        // Compare the normalized artwork layout against actual in-game samples first.
        // The previous histogram comparison discarded pixel positions, so similarly
        // colored purple/blue shards kept receiving the same wrong names even when the
        // source screenshot itself had been added as a sample.  Spatial samples preserve
        // the silhouette and character-art placement after the gold card and equipped tag
        // are removed.
        if (Array.isArray(spatialFeature) && spatialFeature.length > 0) {
            ...
            if (spatialSampleMatches.length > 0) {
                ...
                // Tests using the supplied five game screenshots, including the exact 6%
                // inner crop used by this recognizer, stayed below 0.38 with a different-
                // name margin above 0.11.  Keep a small allowance for browser resizing and
                // JPEG recompression while still rejecting an unrelated unsampled shard.
        ...
        // Real in-game inventory thumbnail color histograms are retained as a fallback
        // for captures where the spatial mask is damaged by severe scaling.
        ...
        // The staggered Road Nightmare shard is visually similar in size to the two
        // 2x4 dream shards.  When an equipped tag covers its upper edge, silhouette
        // extraction can flatten it into a rectangle.  Compare the actual artwork
        // layout before accepting that rectangular result.
        ...
        if (Array.isArray(spatialFeature) && spatialFeature.length > 0 &&
            ...
            // Supplied in-game screenshots keep a margin of at least ~0.18 for the
            // twisted shard, while the two true rectangles remain below ~0.07.
        ...
        // Only override the silhouette result when the artwork match is clearly stronger.
        // This fixes equipped staggered shards whose missing corner is hidden by the tag,
        // without changing correctly recognized one-candidate shapes.
    ...
    async function onCvReady() {
        ...
        try {
            ...
            // Frozen fallback features are identical in every environment, so image
            // recognition can continue even if a browser fails to decode a sample.
...
async function recognizePiecesWithCV(file) {
    ...
    for (let i = 0; i < boxes.length; i++) {
        ...
        // 유니크는 그림 인식 결과를 최종 기준으로 사용한다. 기존에는 그림에서 정확한
        // 이름을 찾았어도 잘못 추출된 모양 후보가 이름과 미리보기를 다시 덮어썼다.
        ...
        // 유니크 그림 이름까지 인식된 경우에는 장착중 태그로 외곽 모양 일부가
        // 가려졌더라도 같은 유니크로 확정한다. 그렇지 않으면 장착중 조각만
        // failedPieces로 분리되어 동일한 일반 조각과 개수가 합쳐지지 않는다.
...
function extractShapeFromImageWithDebug(src, box, bgColor, index, grade) {
    ...
    // 일반/유니크 모두 카드 안쪽 전체를 분석한다. 유니크에서 장착중 태그 아래를
    // 통째로 잘라내면 뒤틀린 기억의 어긋난 한 칸까지 사라져 2x4 직사각형으로
    // 오인식된다. 각 추출기의 초록색 마스크가 태그만 제외하도록 둔다.
...
function findPieceNameByShape(extractedShape) {
    ...
    function solve() {
        ...
        Object.entries(PIECES).forEach(([name, piece]) => {
            if (piece.isUnique) {
                // 유니크 탭의 등급별 입력값을 합산하고, 이미지에서 읽은 유니크 이름을 연결한다.
        ...
        // 현재 쿠키의 유니크 우선순위에 포함되지 않은 조각은 배치 후보에서 제외한다.
        // 보유 유니크가 있어도 우선순위에 맞는 조각이 하나도 없으면 일반 조각만으로 탐색한다.
        ...
        // 우선순위에 맞는 유니크가 여러 종류라면 가장 높은 순위의 종류만 배치 후보로 남긴다.
    ...
    async function search(root, partialSolution = [], currentScore = 0, prioritySets = []) {
        ...
        // 순위: ①목표 세트 달성 개수 → ②유니크 우선순위 → ③총 저항 → ④채운 칸 수
