document.addEventListener('DOMContentLoaded', () => {
    ...
    let lockedCells = new Set(); // 전환 불가 셀
    let piecesToUse = [];
    let isSolving = false;

    const MAX_SOLUTIONS = 10;
    const MAX_TIME_MS = 30000; // 30초 제한
    const PRIORITIZE_HIGH_SCORE = true; // 높은 점수 조각 우선 배치
    ...
    // 포인터 업 뒤에 브라우저가 만드는 클릭을 캡처 단계에서 한 번 더 차단
    ...
    // 동적으로 생성되는 수정창 자체에서 발생한 이벤트가 뒤쪽 화면까지 버블링되지 않게
...
`;
    ...
    function initializeLockedArea() {
        const startRow = 2; // 세로 중앙
        const startCol = 1; // 가로 중앙
        const rows = 3; // 높이
        const cols = 5; // 너비
    ...
    function createGrid() {
        ...
        for (let i = 0; i < GRID_SIZE * GRID_SIZE; i++) {
            ...
            if (lockedCells.has(i)) {
                ...
                cell.classList.add('unlocked'); // 잠금 셀 배치 허용
    ...
    function toggleCell(index) {
        if (isSolving) return;
        if (lockedCells.has(index)) return; // 잠금 셀 전환 금지
    ...
    const COOKIE_SIM_UNIQUE_REFERENCE_META =
        (typeof window !== 'undefined' && window.COOKIE_SIM_UNIQUE_REFERENCE_META) || {};
    const COOKIE_SIM_UNIQUE_PRIORITY =
        (typeof window !== 'undefined' && Array.isArray(window.COOKIE_SIM_UNIQUE_PRIORITY))
            ? window.COOKIE_SIM_UNIQUE_PRIORITY.slice()
            : [];
    const COOKIE_SIM_UNIQUE_NAME_EN = Object.freeze({
        '나비스의 기억': "Navis's Memory",
        '데스파라거스의 기억': "Death Asparagus's Memory",
        '로스베이컨맛 쿠키의 기억': "Roasted Bacon Cookie's Memory",
        '슈가스타의 기억': "Sugarstar's Memory",
        '아르고의 기억': "Argo's Memory",
        '용감한 쿠키의 기억': "GingerBrave's Memory",
        '윈드파라거스 쿠키의 기억': "Wind Asparagus Cookie's Memory",
        '정화된 에메랄딘의 기억': "Purified Emeraldine's Memory",
        '다크초코 쿠키의 기억': "Dark Choco Cookie's Memory",
        '연금술사맛 쿠키의 기억': "Alchemist Cookie's Memory",
        '선데맛 쿠키의 기억': "Sundae Cookie's Memory",
        '오래된 샬롯맛 쿠키의 기억': "Old Shallot Cookie's Memory",
        '블랙베리맛 쿠키의 기억': "Blackberry Cookie's Memory",
        '웨어울프맛 쿠키의 기억': "Werewolf Cookie's Memory",
        '샬롯맛 쿠키의 기억': "Shallot Cookie's Memory",
        '버터밀크맛 쿠키의 기억': "Buttermilk Cookie's Memory",
        '콜라비맛 쿠키의 기억': "Kohlrabi Cookie's Memory",
        ...
        '피닉스페퍼 쿠키의 기억': "Phoenix Pepper Cookie's Memory",
        '폭주한 룽샤맛 쿠키의 기억': "Rampaging Mala Longxia Cookie's Memory",
        '꺼지지 않는 봉화의 기억': 'Everlasting Beacon Memory',
        '마라맛 쿠키의 기억': "Mala Cookie's Memory",
        '룽샤맛 쿠키의 기억': "Mala Longxia Cookie's Memory",
        '크러쉬드페퍼맛 쿠키의 기억': "Crushed Pepper Cookie's Memory",
        '체리맛 쿠키의 기억': "Cherry Cookie's Memory",
        '불야성의 밤의 기억': 'Nightless City Memory',
        '칠리맛 쿠키의 기억': "Chili Pepper Cookie's Memory"
    });

    function cookieSimUniqueDisplayName(uniqueShardName) {
        const name = String(uniqueShardName || '').trim();
        const english = typeof window !== 'undefined' && window.COOKIE_SIM_LANG === 'en';
        return english ? (COOKIE_SIM_UNIQUE_NAME_EN[name] || name) : name;
    }

    if (typeof window !== 'undefined') {
        window.cookieSimUniqueDisplayName = cookieSimUniqueDisplayName;
        window.COOKIE_SIM_UNIQUE_RECOGNIZER_VERSION = 'browser-reference-v5';
    }

    function cookieSimUniquePriorityRank(uniqueShardName) {
        const index = COOKIE_SIM_UNIQUE_PRIORITY.indexOf(uniqueShardName);
        return index >= 0 ? index + 1 : 1000;
    }

    function cookieSimUniqueNamesForShape(shape) {
        const signature = cookieSimShapeSignature(shape || []);
        if (!signature) return [];
        return Object.entries(COOKIE_SIM_UNIQUE_REFERENCE_META)
            .filter(([, meta]) => meta && meta.shapeSignature === signature)
            .map(([uniqueName]) => uniqueName)
            .sort((a, b) => {
                const rankDiff = cookieSimUniquePriorityRank(a) - cookieSimUniquePriorityRank(b);
                return rankDiff || a.localeCompare(b, 'ko');
            });
    }

    function cookieSimUniqueNamesForPiece(pieceName) {
        const template = UNIQUE_PIECE_TEMPLATES[pieceName];
        return template ? cookieSimUniqueNamesForShape(template.shape) : [];
    }

    function cookieSimUniquePieceNameForShard(uniqueShardName) {
        const uniqueName = String(uniqueShardName || '').trim();
        const meta = COOKIE_SIM_UNIQUE_REFERENCE_META[uniqueName];
        const signature = meta && String(meta.shapeSignature || '').trim();
        if (!signature) return null;
        const matched = Object.entries(UNIQUE_PIECE_TEMPLATES)
            .find(([, template]) => cookieSimShapeSignature((template && template.shape) || []) === signature);
        return matched ? matched[0] : null;
    }

    function cookieSimResolveUniqueShardName(candidateNames, currentName) {
        const candidates = Array.from(new Set((candidateNames || []).map(name => String(name || '').trim()).filter(Boolean)));
    ...
    function cookieSimApplyOverflowAlignment(chip) {
        if (!chip) return;
        ...
        chip.style.setProperty('word-break', 'normal', 'important');
        chip.style.setProperty('overflow-wrap', 'normal', 'important');
        chip.style.setProperty('overflow', 'hidden', 'important');
        chip.style.setProperty('text-overflow', 'ellipsis', 'important');

        const updateAlignment = () => {
            const width = Number(chip.clientWidth || 0);
            if (width <= 1) return false;
            chip.style.setProperty('text-align', 'center', 'important');
            const isOverflowing = chip.scrollWidth > width + 1;
            chip.style.setProperty('text-align', isOverflowing ? 'left' : 'center', 'important');
            chip.dataset.csTextOverflow = isOverflowing ? 'true' : 'false';
            return true;
        };

        if (!chip.__csOverflowResizeObserver && typeof ResizeObserver !== 'undefined') {
            chip.__csOverflowResizeObserver = new ResizeObserver(() => updateAlignment());
            chip.__csOverflowResizeObserver.observe(chip);
        }
        ...
        if (typeof requestAnimationFrame === 'function') {
            ...
            setTimeout(updateAlignment, 0);
        }
    }
    if (typeof window !== 'undefined') window.cookieSimApplyOverflowAlignment = cookieSimApplyOverflowAlignment;

    function cookieSimSetUniqueShardNameChip(chip, uniqueShardName, candidateNames = [], onChange = null, options = null) {
        if (!chip) return '';
        const fallback = (typeof window !== 'undefined' && window.COOKIE_SIM_LANG === 'en')
            ? 'Name not recognized'
            : '이름 미인식';
        const candidates = Array.from(new Set((candidateNames || []).map(name => String(name || '').trim()).filter(Boolean)));
        const currentName = String(uniqueShardName || '').trim();
        const preserveCurrent = !!(options && options.preserveCurrent && currentName);
        if (preserveCurrent && !candidates.includes(currentName)) candidates.unshift(currentName);
        const resolvedName = preserveCurrent
            ? currentName
            : cookieSimResolveUniqueShardName(candidates, currentName);
        const label = resolvedName ? cookieSimUniqueDisplayName(resolvedName) : fallback;

        if (chip.__csUniqueNameClickHandler) {
            chip.removeEventListener('click', chip.__csUniqueNameClickHandler);
            chip.__csUniqueNameClickHandler = null;
        }

        chip.replaceChildren();
        chip.textContent = label;
        chip.dataset.uniqueShardName = resolvedName;
        chip.dataset.uniqueCandidateCount = String(candidates.length);
        chip.title = label;
        chip.classList.add('cs-unique-name-chip');
        chip.style.setProperty('line-height', '20px', 'important');
        cookieSimApplyOverflowAlignment(chip);

        if (candidates.length > 1) {
            const clickHandler = (event) => {
                event.preventDefault();
                event.stopPropagation();
                csOpenUniqueShardEditor(
                    chip.dataset.uniqueShardName || resolvedName,
                    candidates,
                    (nextName) => {
                        chip.dataset.uniqueShardName = nextName;
                        const nextLabel = nextName ? cookieSimUniqueDisplayName(nextName) : fallback;
                        chip.textContent = nextLabel;
                        chip.title = nextLabel;
                        cookieSimApplyOverflowAlignment(chip);
                        if (typeof onChange === 'function') onChange(nextName);
                    }
                );
            };
            chip.__csUniqueNameClickHandler = clickHandler;
    ...
    function cookieSimUniqueFeatureDistance(left, right) {
    ...
    function cookieSimRgbToHsv(red, green, blue) {
        ...
        let hue = 0;
        if (delta > 1e-9) {
            if (maxValue === r) hue = ((g - b) / delta) % 6;
            else if (maxValue === g) hue = (b - r) / delta + 2;
            else hue = (r - g) / delta + 4;
            hue /= 6;
            if (hue < 0) hue += 1;
        }
        const saturation = maxValue <= 1e-9 ? 0 : delta / maxValue;
        return [hue, saturation, maxValue];
    }

    function cookieSimExtractUniqueArtFeature(iconRoi) {
        if (!iconRoi || !iconRoi.rows || !iconRoi.cols) return [];
        const hueBins = 18;
        const saturationBins = 4;
        const valueBins = 4;
        const histogramSize = hueBins * saturationBins * valueBins;
        const histogram = Array(histogramSize).fill(0);
        const sums = Array(6).fill(0);
        const squaredSums = Array(6).fill(0);
        let count = 0;
        const rowStep = Math.max(1, Math.floor(iconRoi.rows / 180));
        const colStep = Math.max(1, Math.floor(iconRoi.cols / 180));

        for (let y = 0; y < iconRoi.rows; y += rowStep) {
            for (let x = 0; x < iconRoi.cols; x += colStep) {
                const pixel = iconRoi.ucharPtr(y, x);
                const red = Number(pixel[0]) || 0;
                const green = Number(pixel[1]) || 0;
                const blue = Number(pixel[2]) || 0;
                const isGoldBackground = red > 135 && green > 90 && blue < 165 && (red + green) > 270 && red >= green - 90;
                const isGreenTag = green > 130 && green > red + 20 && green > blue + 20;
                const isWhiteOutline = red > 235 && green > 225 && blue > 200;
                const isDarkOuterEdge = (x < iconRoi.cols * 0.08 || x > iconRoi.cols * 0.92 || y > iconRoi.rows * 0.92) && red < 70 && green < 100 && blue < 130;
                if (isGoldBackground || isGreenTag || isWhiteOutline || isDarkOuterEdge) continue;

                const redN = red / 255;
                const greenN = green / 255;
                const blueN = blue / 255;
                const [hue, saturation, value] = cookieSimRgbToHsv(red, green, blue);
                const hueIndex = Math.min(hueBins - 1, Math.floor(hue * hueBins));
                ...
                const histogramIndex = (hueIndex * saturationBins + saturationIndex) * valueBins + valueIndex;
                histogram[histogramIndex] += 1;
                const values = [redN, greenN, blueN, hue, saturation, value];
                values.forEach((item, index) => {
                    sums[index] += item;
                    squaredSums[index] += item * item;
                });
                count++;
            }
        }
        ...
        if (count <= 0) return [];
        const means = sums.map(value => value / count);
        const deviations = squaredSums.map((value, index) => Math.sqrt(Math.max(0, value / count - means[index] * means[index])));
        return histogram.map(value => value / count).concat(means, deviations);
    }

    function cookieSimUniqueArtDistance(left, right) {
        if (!Array.isArray(left) || !Array.isArray(right) || left.length === 0 || left.length !== right.length) {
            return Number.POSITIVE_INFINITY;
        }
        const histogramSize = 18 * 4 * 4;
        let chiSquare = 0;
        for (let i = 0; i < histogramSize; i++) {
            const a = Number(left[i] || 0);
            const b = Number(right[i] || 0);
            const sum = a + b;
            if (sum > 1e-9) chiSquare += ((a - b) * (a - b)) / sum;
        }
        chiSquare *= 0.5;
        let statsDistance = 0;
        let statsCount = 0;
        for (let i = histogramSize; i < left.length; i++) {
            const diff = Number(left[i] || 0) - Number(right[i] || 0);
            statsDistance += diff * diff;
            statsCount++;
        }
        return chiSquare + (statsCount > 0 ? (statsDistance / statsCount) * 0.5 : 0);
    }

    function cookieSimUniqueArtSampleDistance(feature, meta) {
        if (!meta || !Array.isArray(meta.artSamples) || meta.artSamples.length === 0) {
            return Number.POSITIVE_INFINITY;
        }
        let bestDistance = Number.POSITIVE_INFINITY;
        meta.artSamples.forEach((sample) => {
            const distance = cookieSimUniqueArtDistance(feature, sample);
            if (distance < bestDistance) bestDistance = distance;
        });
        return bestDistance;
    }

    function cookieSimUniqueSpatialSampleDistance(feature, meta) {
        if (!meta || !Array.isArray(meta.spatialSamples) || meta.spatialSamples.length === 0) {
            return Number.POSITIVE_INFINITY;
        }
        let bestDistance = Number.POSITIVE_INFINITY;
        meta.spatialSamples.forEach((sample) => {
            const distance = cookieSimUniqueSpatialDistance(feature, sample);
            if (distance < bestDistance) bestDistance = distance;
        });
        return bestDistance;
    }

    function cookieSimUniqueSpatialDistance(left, right) {
        if (!Array.isArray(left) || !Array.isArray(right) || left.length === 0 || left.length !== right.length || left.length % 4 !== 0) {
            return Number.POSITIVE_INFINITY;
        }
        ...
        for (let offset = 0; offset < left.length; offset += 4) {
            const leftAlpha = Number(left[offset + 3] || 0);
            const rightAlpha = Number(right[offset + 3] || 0);
            const alphaDiff = leftAlpha - rightAlpha;
            occupancyDistance += alphaDiff * alphaDiff;
            const unionAlpha = Math.max(leftAlpha, rightAlpha);
            if (unionAlpha <= 1e-9) continue;
            for (let channel = 0; channel < 3; channel++) {
                const diff = Number(left[offset + channel] || 0) - Number(right[offset + channel] || 0);
                colorDistance += diff * diff * unionAlpha;
            }
            colorWeight += unionAlpha * 3;
        }
        const occupancyMean = occupancyDistance / Math.max(1, pixelCount);
        const colorMean = colorDistance / Math.max(1e-9, colorWeight);
        return occupancyMean * 1.5 + colorMean;
    }

    function cookieSimUniqueCardDistance(left, right, featureSize = 24) {
        if (!Array.isArray(left) || !Array.isArray(right) ||
            left.length !== featureSize * featureSize * 4 || right.length !== left.length) {
            return Number.POSITIVE_INFINITY;
        }
            ...
            for (let shiftX = -2; shiftX <= 2; shiftX++) {
                ...
                for (let y = 0; y < featureSize; y++) {
                    for (let x = 0; x < featureSize; x++) {
                        const leftOffset = (y * featureSize + x) * 4;
                        const rightX = x - shiftX;
                        const rightY = y - shiftY;
                        let rightRed = 0, rightGreen = 0, rightBlue = 0, rightAlpha = 0;
                        if (rightX >= 0 && rightX < featureSize && rightY >= 0 && rightY < featureSize) {
                            const rightOffset = (rightY * featureSize + rightX) * 4;
                            rightRed = Number(right[rightOffset] || 0);
                            rightGreen = Number(right[rightOffset + 1] || 0);
                            rightBlue = Number(right[rightOffset + 2] || 0);
                            rightAlpha = Number(right[rightOffset + 3] || 0);
                        }
                        const leftAlpha = Number(left[leftOffset + 3] || 0);
                        const alphaDiff = leftAlpha - rightAlpha;
                        occupancyDistance += alphaDiff * alphaDiff;
                        const unionAlpha = Math.max(leftAlpha, rightAlpha);
                        if (unionAlpha <= 1e-9) continue;
                        const redDiff = Number(left[leftOffset] || 0) - rightRed;
                        const greenDiff = Number(left[leftOffset + 1] || 0) - rightGreen;
                        const blueDiff = Number(left[leftOffset + 2] || 0) - rightBlue;
                        colorDistance += (redDiff * redDiff + greenDiff * greenDiff + blueDiff * blueDiff) * unionAlpha;
                        colorWeight += unionAlpha * 3;
                    }
                }
                const occupancyMean = occupancyDistance / (featureSize * featureSize);
                const colorMean = colorDistance / Math.max(1e-9, colorWeight);
                bestDistance = Math.min(bestDistance, occupancyMean * 1.2 + colorMean);
            }
        }
        return bestDistance;
    }

    function cookieSimUniqueCardSampleDistance(feature, meta) {
        if (!meta || !Array.isArray(meta.cardSamples) || meta.cardSamples.length === 0) {
            return Number.POSITIVE_INFINITY;
        }
        let bestDistance = Number.POSITIVE_INFINITY;
        meta.cardSamples.forEach((sample) => {
            bestDistance = Math.min(bestDistance, cookieSimUniqueCardDistance(feature, sample));
        });
        return bestDistance;
    }

    function cookieSimExtractUniqueCardFeature(iconRoi, featureSize = 24) {
        if (!iconRoi || !iconRoi.rows || !iconRoi.cols) return [];
        const resized = new cv.Mat();
        cv.resize(iconRoi, resized, new cv.Size(featureSize, featureSize), 0, 0, cv.INTER_AREA);
        const feature = [];
        for (let y = 0; y < featureSize; y++) {
            for (let x = 0; x < featureSize; x++) {
                const pixel = resized.ucharPtr(y, x);
                const red = (Number(pixel[0]) || 0) / 255;
                const green = (Number(pixel[1]) || 0) / 255;
                const blue = (Number(pixel[2]) || 0) / 255;
                const isGoldBackground = (
                    red > 0.53 && green > 0.36 && blue < 0.68 &&
                    red + green > 1.05 && red >= green - 0.35
                );
                const isGreenTag = green > 0.45 && green > red + 0.05 && green > blue + 0.05;
                const isTagText = y < featureSize * 0.28 && red > 0.72 && green > 0.72 && blue > 0.72;
                const isOuterEdge = x === 0 || x === featureSize - 1 || y === featureSize - 1;
                const alpha = (isGoldBackground || isGreenTag || isTagText || isOuterEdge) ? 0 : 1;
                feature.push(red * alpha, green * alpha, blue * alpha, alpha);
            }
        }
        resized.delete();
        return feature;
    ...
    function cookieSimExtractUniqueSpatialFeature(iconRoi, featureSize = 20) {
        if (!iconRoi || !iconRoi.rows || !iconRoi.cols) return [];
        const rawMask = cv.Mat.zeros(iconRoi.rows, iconRoi.cols, cv.CV_8UC1);
        for (let y = 0; y < iconRoi.rows; y++) {
            for (let x = 0; x < iconRoi.cols; x++) {
                const pixel = iconRoi.ucharPtr(y, x);
                const red = Number(pixel[0]) || 0;
                const green = Number(pixel[1]) || 0;
                const blue = Number(pixel[2]) || 0;
                const isGoldBackground = (
                    red > 140 && green > 100 && blue < 150 &&
                    (red + green) > 300 && red >= green - 75
                );
                const isGreenTag = green > 120 && green > red + 15 && green > blue + 15;
                const nearOuterEdge = x <= 1 || x >= iconRoi.cols - 2 || y >= iconRoi.rows - 2;
                if (!isGoldBackground && !isGreenTag && !nearOuterEdge) {
        ...
        const cleanMask = cv.Mat.zeros(iconRoi.rows, iconRoi.cols, cv.CV_8UC1);
        let keptContourCount = 0;
        for (let index = 0; index < contours.size(); index++) {
            const contour = contours.get(index);
            const area = cv.contourArea(contour);
            if (area < 5) continue;
            const rect = cv.boundingRect(contour);
            const touchesLeft = rect.x <= 1;
            const touchesRight = rect.x + rect.width >= iconRoi.cols - 1;
            const touchesBottom = rect.y + rect.height >= iconRoi.rows - 1;
            if (touchesLeft || touchesRight || touchesBottom) continue;
            cv.drawContours(cleanMask, contours, index, new cv.Scalar(255), cv.FILLED);
            keptContourCount++;
        }
        if (keptContourCount === 0) rawMask.copyTo(cleanMask);
        contours.delete();
        hierarchy.delete();
        rawMask.delete();

        let minX = iconRoi.cols;
        let maxX = -1;
        let minY = iconRoi.rows;
        let maxY = -1;
        for (let y = 0; y < cleanMask.rows; y++) {
            for (let x = 0; x < cleanMask.cols; x++) {
                if (cleanMask.ucharPtr(y, x)[0] <= 128) continue;
        ...
        const innerSize = Math.max(1, featureSize - 2);
        ...
        const rect = new cv.Rect(minX, minY, cropWidth, cropHeight);
        const colorCrop = iconRoi.roi(rect);
        const maskCrop = cleanMask.roi(rect);
        const resizedColor = new cv.Mat();
        const resizedMask = new cv.Mat();
        cv.resize(colorCrop, resizedColor, new cv.Size(resizedWidth, resizedHeight), 0, 0, cv.INTER_AREA);
        cv.resize(maskCrop, resizedMask, new cv.Size(resizedWidth, resizedHeight), 0, 0, cv.INTER_AREA);
        colorCrop.delete();
        maskCrop.delete();
        cleanMask.delete();

        const offsetX = Math.floor((featureSize - resizedWidth) / 2);
        const offsetY = Math.floor((featureSize - resizedHeight) / 2);
        const feature = [];
        for (let targetY = 0; targetY < featureSize; targetY++) {
            for (let targetX = 0; targetX < featureSize; targetX++) {
                const sourceX = targetX - offsetX;
                const sourceY = targetY - offsetY;
                if (sourceX < 0 || sourceY < 0 || sourceX >= resizedWidth || sourceY >= resizedHeight) {
                    feature.push(0, 0, 0, 0);
                    continue;
                }
                const alpha = (Number(resizedMask.ucharPtr(sourceY, sourceX)[0]) || 0) / 255;
                const pixel = resizedColor.ucharPtr(sourceY, sourceX);
                feature.push(
                    ((Number(pixel[0]) || 0) / 255) * alpha,
                    ((Number(pixel[1]) || 0) / 255) * alpha,
                    ((Number(pixel[2]) || 0) / 255) * alpha,
                    alpha
                );
            }
        }
    ...
    let cookieSimUniqueReferenceInitPromise = null;

    function cookieSimLoadReferenceImage(source) {
        return new Promise((resolve, reject) => {
            const image = new Image();
            image.onload = () => resolve(image);
            image.onerror = () => reject(new Error('유니크 조각 샘플 이미지를 불러오지 못했습니다.'));
            image.src = source;
        });
    }

    async function cookieSimInitializeUniqueReferenceSamples() {
        if (cookieSimUniqueReferenceInitPromise) return cookieSimUniqueReferenceInitPromise;

        cookieSimUniqueReferenceInitPromise = (async () => {
            for (const meta of Object.values(COOKIE_SIM_UNIQUE_REFERENCE_META)) {
                if (!meta || !Array.isArray(meta.sampleImageUris) || meta.sampleImageUris.length === 0) {
                    continue;
                }

                const browserCardSamples = [];
                const browserSpatialSamples = [];
                const browserArtSamples = [];

                for (const source of meta.sampleImageUris) {
                    const image = await cookieSimLoadReferenceImage(source);
                    const sampleMat = cv.imread(image);
                    try {
                        const cardSample = cookieSimExtractUniqueCardFeature(sampleMat);
                        const spatialSample = cookieSimExtractUniqueSpatialFeature(sampleMat);
                        const artSample = cookieSimExtractUniqueArtFeature(sampleMat);
                        if (Array.isArray(cardSample) && cardSample.length > 0) browserCardSamples.push(cardSample);
                        if (Array.isArray(spatialSample) && spatialSample.length > 0) browserSpatialSamples.push(spatialSample);
                        if (Array.isArray(artSample) && artSample.length > 0) browserArtSamples.push(artSample);
                    } finally {
                        sampleMat.delete();
                    }
                }

                if (browserCardSamples.length > 0) meta.cardSamples = browserCardSamples;
                if (browserSpatialSamples.length > 0) meta.spatialSamples = browserSpatialSamples;
                if (browserArtSamples.length > 0) meta.artSamples = browserArtSamples;
                meta.sampleFeatureSource = 'browser-opencvjs-ready';
            }

            if (typeof window !== 'undefined') {
                window.COOKIE_SIM_UNIQUE_REFERENCE_READY = true;
            }
        })();

        return cookieSimUniqueReferenceInitPromise;
    }

    function cookieSimRecognizeUniqueShard(shape, visualFeature, artFeature, spatialFeature, cardFeature) {
        const earlySignature = cookieSimShapeSignature(shape || []);
        const earlyShapeCandidates = Object.entries(COOKIE_SIM_UNIQUE_REFERENCE_META)
            .filter(([, meta]) => meta && meta.shapeSignature === earlySignature);
        if (earlyShapeCandidates.length === 1) {
            const [onlyName, onlyMeta] = earlyShapeCandidates[0];
            if (!onlyMeta || !Array.isArray(onlyMeta.cardSamples) || onlyMeta.cardSamples.length === 0) {
                return onlyName;
            }
        }

        if (Array.isArray(cardFeature) && cardFeature.length > 0) {
            const cardMatches = Object.entries(COOKIE_SIM_UNIQUE_REFERENCE_META)
                .map(([uniqueName, meta]) => ({
                    uniqueName,
                    distance: cookieSimUniqueCardSampleDistance(cardFeature, meta)
                }))
                .filter((item) => Number.isFinite(item.distance))
                .sort((left, right) => left.distance - right.distance);
            if (cardMatches.length > 0) {
                const bestCardMatch = cardMatches[0];
                const secondCardMatch = cardMatches[1] || { distance: Number.POSITIVE_INFINITY };
                const cardGap = secondCardMatch.distance - bestCardMatch.distance;
                if (bestCardMatch.distance <= 0.46 && cardGap >= 0.04) {
                    return bestCardMatch.uniqueName;
                }
            }
        }

        const signature = cookieSimShapeSignature(shape || []);
        const shapeCandidates = Object.entries(COOKIE_SIM_UNIQUE_REFERENCE_META)
            .filter(([, meta]) => meta && meta.shapeSignature === signature);

        let shapeBestName = null;
        if (shapeCandidates.length === 1) {
            shapeBestName = shapeCandidates[0][0];
        } else if (shapeCandidates.length > 1 && Array.isArray(visualFeature) && visualFeature.length > 0) {
            let bestDistance = Number.POSITIVE_INFINITY;
            shapeCandidates.forEach(([uniqueName, meta]) => {
                const distance = cookieSimUniqueFeatureDistance(visualFeature, meta.feature);
                if (distance < bestDistance) {
                    bestDistance = distance;
                    shapeBestName = uniqueName;
                }
            });
        ...
        if (Array.isArray(spatialFeature) && spatialFeature.length > 0) {
            const spatialSampleMatches = Object.entries(COOKIE_SIM_UNIQUE_REFERENCE_META)
                .map(([uniqueName, meta]) => ({
                    uniqueName,
                    distance: cookieSimUniqueSpatialSampleDistance(spatialFeature, meta)
                }))
                .filter((item) => Number.isFinite(item.distance))
                .sort((left, right) => left.distance - right.distance);
            if (spatialSampleMatches.length > 0) {
                const spatialBest = spatialSampleMatches[0];
                const spatialSecond = spatialSampleMatches[1] || { distance: Number.POSITIVE_INFINITY };
                const spatialGap = spatialSecond.distance - spatialBest.distance;
                const strongSpatialSampleMatch =
                    spatialBest.distance <= 0.40 && spatialGap >= 0.075;
                if (strongSpatialSampleMatch) return spatialBest.uniqueName;
            }
        }

        if (Array.isArray(artFeature) && artFeature.length > 0) {
            const sampleMatches = Object.entries(COOKIE_SIM_UNIQUE_REFERENCE_META)
                .map(([uniqueName, meta]) => ({
                    uniqueName,
                    distance: cookieSimUniqueArtSampleDistance(artFeature, meta)
                }))
                .filter((item) => Number.isFinite(item.distance))
                .sort((left, right) => left.distance - right.distance);
            if (sampleMatches.length > 0) {
                const sampleBest = sampleMatches[0];
                const sampleSecond = sampleMatches[1] || { distance: Number.POSITIVE_INFINITY };
                const sampleGap = sampleSecond.distance - sampleBest.distance;
                const strongSampleMatch =
                    sampleBest.distance <= 0.22 ||
                    (sampleBest.distance <= 0.34 && sampleGap >= 0.055);
                if (strongSampleMatch) return sampleBest.uniqueName;
            }
        }

        const rectangularDreamNames = ['꿈열차에 실린 기억', '꿈세계의 기억'];
        const twistedName = '로드 나이트메어의 뒤틀린 기억';
        if (shapeBestName === twistedName) return twistedName;
        if (Array.isArray(spatialFeature) && spatialFeature.length > 0 &&
            (!shapeBestName || rectangularDreamNames.includes(shapeBestName))) {
            const twistedMeta = COOKIE_SIM_UNIQUE_REFERENCE_META[twistedName];
            const twistedDistance = cookieSimUniqueSpatialDistance(
                spatialFeature,
                twistedMeta && twistedMeta.spatialFeature
            );
            const rectangularDistance = Math.min(...rectangularDreamNames.map((name) => {
                const meta = COOKIE_SIM_UNIQUE_REFERENCE_META[name];
                return cookieSimUniqueSpatialDistance(spatialFeature, meta && meta.spatialFeature);
            }));
            if (Number.isFinite(twistedDistance) && Number.isFinite(rectangularDistance) &&
                rectangularDistance - twistedDistance >= 0.12) {
                return twistedName;
            }
        }

        if (!Array.isArray(artFeature) || artFeature.length === 0) return shapeBestName;
        const artMatches = Object.entries(COOKIE_SIM_UNIQUE_REFERENCE_META)
            .map(([uniqueName, meta]) => ({
                uniqueName,
                distance: cookieSimUniqueArtDistance(artFeature, meta && meta.artFeature)
            }))
            .filter(item => Number.isFinite(item.distance))
            .sort((left, right) => left.distance - right.distance);
        if (artMatches.length === 0) return shapeBestName;

        const artBest = artMatches[0];
        const artSecond = artMatches[1] || { distance: Number.POSITIVE_INFINITY };
        if (!shapeBestName) return artBest.uniqueName;
        if (artBest.uniqueName === shapeBestName) return shapeBestName;

        const shapeArtMatch = artMatches.find(item => item.uniqueName === shapeBestName);
        const shapeArtDistance = shapeArtMatch ? shapeArtMatch.distance : Number.POSITIVE_INFINITY;
        const confidenceGap = artSecond.distance - artBest.distance;
        const clearlyBetterArtwork = artBest.distance <= shapeArtDistance * 0.78 && confidenceGap >= 0.07;
        return clearlyBetterArtwork ? artBest.uniqueName : shapeBestName;
    }

    function cookieSimBuildRecognizedUniqueInventory() {
        const inventory = {};
        (csRecognizedResults || []).forEach((item) => {
            if (!item || item.grade !== 'unique' || !item.pieceName || !item.uniqueShardName) return;
            const count = Math.max(0, parseInt(item.count || 0, 10) || 0);
            if (count <= 0) return;
            inventory[item.pieceName] = inventory[item.pieceName] || [];
            for (let i = 0; i < count; i++) inventory[item.pieceName].push(item.uniqueShardName);
        });
        Object.values(inventory).forEach((items) => {
            items.sort((a, b) => cookieSimUniquePriorityRank(a) - cookieSimUniquePriorityRank(b));
        });
        return inventory;
    }
    ...
    async function onCvReady() {
        uploadStatus.textContent = '유니크 조각 샘플 준비 중...';
        uploadStatus.style.color = '#667eea';

        try {
            await cookieSimInitializeUniqueReferenceSamples();
            ...
            console.error('Unique reference initialization failed:', error);
            uploadStatus.textContent = '이미지 분석 준비 완료';
            uploadStatus.style.color = '#10b981';
            uploadBtn.style.pointerEvents = 'auto';
            uploadBtn.style.cursor = 'pointer';
    ...
    function showSetSelectionModal(imagesData) {
        return new Promise((resolve) => {
            ...
            modalTitle.textContent = ((typeof window !== 'undefined' && window.COOKIE_SIM_LANG === 'en') ? 'Set Selection by Photo' : '사진별 세트 선택');
            ...
            modalTitle.style.boxSizing = 'border-box';
            modalTitle.style.marginBottom = '10px';
            modalTitle.style.background = 'transparent';
            modalTitle.style.color = csSelectFieldText();
            modalTitle.style.fontSize = '12px';
            modalTitle.style.fontWeight = '800';
            ...
            tabButtons.style.display = 'flex';
            tabButtons.style.gap = '5px';
            tabButtons.style.marginBottom = '0px';
            ...
            tabButtons.style.paddingTop = '4px';
            tabButtons.style.paddingBottom = '2px';

            const tabContents = document.createElement('div');
            tabContents.style.minHeight = '0';

            let activeTabIndex = 0;
            const imageSetSelectors = []; // 이미지별 세트 선택기
        ...
        btn.style.fontFamily = 'Arial, sans-serif';
            ...
            imagesData.forEach((imageData, imageIndex) => {
                ...
                statEditHint.style.color = CS_DARK_UI ? '#9ca3af' : '#6b7280';
                ...
                const sortedPieces = [...pieces].sort((a, b) => {
                    ...
                    if (aHasIssue && !bHasIssue) return -1; // 문제 조각 우선
                    if (!aHasIssue && bHasIssue) return 1; // 문제 조각 우선
                    return 0; // 동일 상태 순서 유지
                });
                ...
                sortedPieces.forEach((data, sortedIndex) => {
                    ...
                    if (pieceName === null && failedPieces) {
                        ...
                        failedPieces.forEach((failedPiece, failedIndex) => {
                            ...
                            // 등급별 배경색 설정
                            let gradeColor = 'rgba(200, 200, 200, 0.2)'; // 인식 실패 회색
                    ...
                    pieceBlock.style.alignItems = 'center';
                            ...
                            pieceBlock.style.height = '156px';
                            pieceBlock.style.minHeight = pieceBlock.style.height;
                            pieceBlock.style.maxHeight = pieceBlock.style.height;
                            ...
                            pieceBlock.dataset.uniqueShardName = failedPiece.uniqueShardName || '';
            ...
            setChip.style.color = '#10b981';
                            ...
                            if (failedGrade !== 'unique') {
                                ...
                                const failedUniqueCandidates = cookieSimUniqueNamesForPiece(defaultPieceName || failedPiece.pieceName);
                                const resolvedFailedUniqueName = cookieSimSetUniqueShardNameChip(
                                    failedStatChip,
                                    failedPiece.uniqueShardName,
                                    failedUniqueCandidates,
                                    (nextName) => {
                                        failedPiece.uniqueShardName = nextName;
                                        pieceBlock.dataset.uniqueShardName = nextName;
                                    },
                                    { preserveCurrent: true }
                                );
                                failedPiece.uniqueShardName = resolvedFailedUniqueName;
                                pieceBlock.dataset.uniqueShardName = resolvedFailedUniqueName;
                            }
                            ...
                            selectButton.addEventListener('click', () => {
                                ...
                                modalTitle.textContent = '조각 선택';
                                modalTitle.style.margin = '0 0 10px 0';
                                modalTitle.style.color = CS_DARK_UI ? '#f3f4f6' : '#374151';
                                modalTitle.style.fontSize = '12px';
                                modalTitle.style.fontWeight = '800';
                                modalContent.appendChild(modalTitle);
                                ...
                                allPieces.forEach(pName => {
                                    ...
                                    pieceCard.addEventListener('click', () => {
                                        ...
                                        pieceBlock.dataset.selectedGrade = failedGrade;
                                        if (failedGrade === 'unique') {
                                            const nextCandidates = cookieSimUniqueNamesForPiece(pName);
                                            const nextUniqueName = cookieSimSetUniqueShardNameChip(
                                                failedStatChip,
                                                failedPiece.uniqueShardName,
                                                nextCandidates,
                                                (selectedName) => {
                                                    failedPiece.uniqueShardName = selectedName;
                                                    pieceBlock.dataset.uniqueShardName = selectedName;
                                                }
                                            );
                                            failedPiece.uniqueShardName = nextUniqueName;
                                            pieceBlock.dataset.uniqueShardName = nextUniqueName;
                                        }
                                        pieceBlock.style.border = CS_DARK_UI ? '0' : `1px solid ${gradeBorderColor}`;
                    ...
                    pieceBlock.dataset.uniqueShardName = data.uniqueShardName || '';
                    pieceBlock.style.marginBottom = '0';
                    pieceBlock.style.padding = '4px';
                    pieceBlock.style.background = gradeColor;
                    pieceBlock.style.borderRadius = '8px';
                    ...
                    pieceBlock.style.alignItems = 'center';
                    pieceBlock.style.gap = '4px';
                    pieceBlock.style.width = '132px'; // 한 줄 5개
                    pieceBlock.style.minWidth = '132px';
                    pieceBlock.style.boxSizing = 'border-box';
                    pieceBlock.style.height = '156px';
                    pieceBlock.style.minHeight = pieceBlock.style.height;
                    ...
                    if (grade !== 'unique') {
                        ...
                        const normalUniqueCandidates = cookieSimUniqueNamesForPiece(data.pieceName || pieceName);
                        const resolvedNormalUniqueName = cookieSimSetUniqueShardNameChip(
                            normalStatChip,
                            data.uniqueShardName,
                            normalUniqueCandidates,
                            (nextName) => {
                                data.uniqueShardName = nextName;
                                pieceBlock.dataset.uniqueShardName = nextName;
                            },
                            { preserveCurrent: true }
                        );
                        data.uniqueShardName = resolvedNormalUniqueName;
                        pieceBlock.dataset.uniqueShardName = resolvedNormalUniqueName;
                    }
                    ...
                    if (templateData) {
                        ...
                        const tempPiece = {
                            shape: templateData.shape,
                            color: '#999999' // 세트 선택 전 회색
                        };
                        const preview = createPiecePreview(tempPiece);
                        preview.style.flex = '0 0 auto';
                        preview.style.transform = 'scale(0.7)'; // 미리보기 축소
                        ...
                        const bindNormalPreviewEdit = (targetPreview) => {
                            ...
                            targetPreview.addEventListener('click', () => {
                                openCookieSimPiecePicker(data.pieceName || pieceName, (newPieceName) => {
                                    ...
                                    if (grade === 'unique') {
                                        const nextCandidates = cookieSimUniqueNamesForPiece(newPieceName);
                                        const nextUniqueName = cookieSimSetUniqueShardNameChip(
                                            normalStatChip,
                                            data.uniqueShardName,
                                            nextCandidates,
                                            (selectedName) => {
                                                data.uniqueShardName = selectedName;
                                                pieceBlock.dataset.uniqueShardName = selectedName;
                                            }
                                        );
                                        data.uniqueShardName = nextUniqueName;
                                        pieceBlock.dataset.uniqueShardName = nextUniqueName;
                                    }
            ...
            countBadge.style.color = csRecognizedGradeTextColor(grade);
                ...
                function csCreateManualPieceCard(pName, gradeKey) {
                    const initialUniqueCandidates = gradeKey === 'unique' ? cookieSimUniqueNamesForPiece(pName) : [];
                    const initialUniqueName = gradeKey === 'unique'
                        ? cookieSimResolveUniqueShardName(initialUniqueCandidates, null)
                        : null;
                    const manualData = {
                        pieceName: pName,
                        grade: gradeKey,
                        count: 1,
                        stats: null,
                        uniqueShardName: initialUniqueName,
                        deleted: false
                    };
                    ...
                    card.dataset.uniqueShardName = manualData.uniqueShardName || '';
                    ...
                    if (gradeKey !== 'unique') {
                        ...
                        const resolvedManualUniqueName = cookieSimSetUniqueShardNameChip(
                            manualStatChip,
                            manualData.uniqueShardName,
                            initialUniqueCandidates,
                            (nextName) => {
                                manualData.uniqueShardName = nextName;
                                card.dataset.uniqueShardName = nextName;
                            }
                        );
                        manualData.uniqueShardName = resolvedManualUniqueName;
                        card.dataset.uniqueShardName = resolvedManualUniqueName;
                    }
                                ...
                                openCookieSimPiecePicker(manualData.pieceName, (newPieceName) => {
                                    ...
                                    if (gradeKey === 'unique') {
                                        ...
                                        const nextUniqueName = cookieSimSetUniqueShardNameChip(
                                            ...
                                            nextCandidates,
                                            (selectedName) => {
                                                manualData.uniqueShardName = selectedName;
                                                card.dataset.uniqueShardName = selectedName;
                                            }
                                        );
                                        manualData.uniqueShardName = nextUniqueName;
                                        card.dataset.uniqueShardName = nextUniqueName;
                                    }
                                    const newPreview = createPiecePreview({ shape: newTemplate.shape, color: '#999999' });
                                    newPreview.style.flex = '0 0 auto';
                                    newPreview.style.transform = 'scale(0.7)';
                    ...
                    if (!selectedSet) {
                        ...
                        imageData.pieces.forEach((piece, pieceIndex) => {
                            ...
                            if (piece.pieceName === null && piece.failedPieces) {
                                    ...
                                    if (pieceBlock) {
                                        ...
                                        if (selectedPiece) {
                                            ...
                                            results.push({
                                                ...
                                                grade: selectedGrade,
                                                count: selectedCount,
                                                uniqueShardName: failedPiece.uniqueShardName || null,
                                ...
                                results.push({
                                    ...
                                    grade: selectedGrade,
                                    count: selectedCount,
                                    uniqueShardName: piece.uniqueShardName || null,
                        ...
                        (imageData.manualPieces || []).forEach((manualPiece) => {
                            ...
                            results.push({
                                ...
                                count: selectedCount,
                                uniqueShardName: manualPiece.uniqueShardName || null,
        ...
        try {
            ...
            for (const selection of selections) {
                ...
                finalResults.push({
                    ...
                    uniqueShardName: selection.uniqueShardName || null,
    ...
    try {
        ...
        for (let y = y0; y < y1; y += stride) {
            for (let x = x0; x < x1; x += stride) {
                ...
                const r = px[0], g = px[1], b = px[2];
                const hsv = csRgbToHsv(r, g, b);
                if (hsv.v < 0.55) continue; // 어두운 프레임 제외
                if (bg) { // 등급 배경색 제외
                    const d = Math.abs(r - bg.r) + Math.abs(g - bg.g) + Math.abs(b - bg.b);
                    if (d < 90) continue;
                }
                if (hsv.h >= 195 && hsv.h <= 235 && hsv.s < 0.32 && hsv.v < 0.85) continue; // 회색 계열 제외
                if (hsv.h >= 88 && hsv.h <= 145 && hsv.s >= 0.28 && hsv.v >= 0.5 && hsv.v <= 0.92) continue; // 장착중 태그·초록 세트 표식 제외
                if (hsv.s < 0.06 && hsv.v < 0.9) continue; // 저채도 잡음 제외
                samples.push(hsv);
            }
        }
...
function csOpenStatEditor(currentStats, count, onSave) {
    ...
    desc.textContent = en ? 'If there are multiple shards with the same shape, you can choose stats for each one in order below.' : '같은 모양 조각이 여러 개면 아래에서 순서대로 스탯을 따로 고를 수 있습니다.';
    ...
    const makeSelect = (value) => {
        ...
        // 스탯 수정 드롭다운은 흰색 수정창 안쪽 흐름에 포함
        // 구형 고정 배경 모바일 터치 충돌
        // 드롭다운이 바로 닫히거나 여러 번 눌러야 열리는 문제가 생겨 제거
        ...
        const items = Array.from(select.options).map(opt => {
            ...
            const chooseItem = (event) => {
                ...
                syncActive();
                closeInfo.close();
                // 실제 조각 스탯 반영은 아래 '저장' 버튼을 눌렀을 때만 수행
            };
...
function csOpenUniqueShardEditor(currentName, candidateNames, onSave) {
    const en = (typeof window !== 'undefined' && window.COOKIE_SIM_LANG === 'en');
    const candidates = Array.from(new Set((candidateNames || [])
        .map(name => String(name || '').trim())
        .filter(Boolean)));
    if (candidates.length === 0) return;

    let selectedName = cookieSimResolveUniqueShardName(candidates, currentName) || candidates[0];

    const modal = document.createElement('div');
    modal.className = 'cs-unique-editor-modal';
    modal.style.position = 'fixed';
    modal.style.zIndex = '2000';
    modal.style.left = '0';
    modal.style.top = '0';
    ...
    const box = document.createElement('div');
    box.className = 'cs-unique-editor-box';
    box.style.width = 'min(520px, 92vw)';
    box.style.maxHeight = '88dvh';
    box.style.overflowY = 'auto';
    box.style.overflowX = 'visible';
    ...
    const title = document.createElement('div');
    title.textContent = en ? 'Edit unique shard' : '유니크 조각 수정';
    title.style.fontWeight = '800';
    title.style.fontSize = '13px';
    title.style.marginBottom = '6px';
    title.style.color = modalTextColor;
    ...
    const desc = document.createElement('div');
    desc.textContent = en
        ? 'Choose the unique shard name for this shape below.'
        : '같은 모양의 유니크 조각 중에서 사용할 조각을 아래에서 선택할 수 있어요.';
    ...
    label.textContent = cookieSimUniqueDisplayName(selectedName);
    ...
    const items = candidates.map((name) => {
        const item = document.createElement('button');
        item.type = 'button';
        item.textContent = cookieSimUniqueDisplayName(name);
        item.dataset.value = name;
        item.title = cookieSimUniqueDisplayName(name);
        item.style.display = 'block';
        item.style.width = '100%';
        item.style.padding = '9px 12px';
        item.style.border = 'none';
        ...
        item.addEventListener('mouseleave', () => {
            item.style.background = CS_DARK_UI ? '#303236' : '#ffffff';
            item.style.color = CS_DARK_UI ? '#f3f4f6' : '#374151';
            item.style.fontWeight = item.dataset.value === selectedName ? '800' : '700';
        });
        ...
        const chooseItem = (event) => {
            ...
            selectedName = name;
            label.textContent = cookieSimUniqueDisplayName(name);
            syncActive();
            closeMenu();
        };
        item.addEventListener('pointerdown', (event) => {
    ...
    const syncActive = () => {
        items.forEach((item) => {
            const active = item.dataset.value === selectedName;
            item.style.fontWeight = active ? '800' : '700';
            item.style.color = active
                ? (CS_DARK_UI ? '#ffffff' : '#111827')
                : (CS_DARK_UI ? '#f3f4f6' : '#374151');
        });
    };
    ...
    saveBtn.style.color = 'white';
    saveBtn.addEventListener('click', () => {
        if (typeof onSave === 'function') onSave(selectedName);
        destroy();
    });
...
async function recognizePiecesWithCV(file) {
    ...
    // =====================================================
    // 조각 인식 과정 시각화 데이터
    // 깃허브 페이지 데이터 수집 전용 모드
    const debugData = [];

    // 4. 각 박스에서 조각 패턴 추출 및 매칭
    const pieceCounts = {};
    const pieceStatByKey = {}; // 조각 이름·등급별 스탯 개수
    const failedPieces = []; // 매칭 실패한 조각들

    for (let i = 0; i < boxes.length; i++) {
        const box = boxes[i];

        // 배경색 등급 판별: 파랑·보라·빨강/노랑
        let { grade, bgColor } = detectGradeFromBox(src, box);

        // 그리드 분석으로 조각 모양 추출 + 디버그용 캔버스 생성
        const { shape: extractedShape, hasGreenTag, matchScore, uniqueVisualFeature, uniqueArtFeature, uniqueSpatialFeature, uniqueCardFeature, debug } = extractShapeFromImageWithDebug(src, box, bgColor, i, grade);

        // 추출한 모양으로 조각 이름 찾기 (템플릿 매칭)
        // 유니크 이름 그림 인식 결과 최우선
        let pieceName = findPieceNameByShape(extractedShape);
        const isUniqueCard = isCookieSimUniqueGoldBg(bgColor) || !!(pieceName && UNIQUE_PIECE_TEMPLATES[pieceName]);
        if (isUniqueCard) grade = 'unique';
        const uniqueShardName = isUniqueCard
            ? cookieSimRecognizeUniqueShard(extractedShape, uniqueVisualFeature, uniqueArtFeature, uniqueSpatialFeature, uniqueCardFeature)
            : null;
        if (uniqueShardName) {
            const canonicalUniquePieceName = cookieSimUniquePieceNameForShard(uniqueShardName);
            if (canonicalUniquePieceName) pieceName = canonicalUniquePieceName;
        }

        // 디버그 정보에 인식 결과 추가
        debug.info += `\n결과: ${pieceName ? ` ${pieceName} (${grade})` : ' 인식 실패'}`;
        debug.info += `\n추출된 shape: ${JSON.stringify(extractedShape)}`;
        if (uniqueShardName) debug.info += `\n유니크 그림: ${uniqueShardName}`;
        debugData.push(debug);

        // 조각 내부 색상으로 스탯(설탕유리 종류) 분류해 기록
        const statName = csSampleStatFromBox(src, box, bgColor, extractedShape);

        // 장착중 태그 조각 고신뢰도 자동 확정
        // 저신뢰도·모양 미검출 조각 확인 목록
        // 유니크 이름 인식 시 장착중 외곽 가림 허용
        // 인식 실패 조각 별도 집계
        const recognizedUniqueIsReliable = grade === 'unique' && !!uniqueShardName && !!pieceName;
        const taggedMatchIsReliable = recognizedUniqueIsReliable || !hasGreenTag ||
        ...
        if (!pieceName || !taggedMatchIsReliable) {
            failedPieces.push({
                ...
                uniqueShardName: uniqueShardName || null,
                stats: statName ? { [statName]: 1 } : null
            });
        } else {
            const key = grade === 'unique'
                ? `${pieceName}-${grade}|||${uniqueShardName || ''}`
                : `${pieceName}-${grade}`;
            pieceCounts[key] = (pieceCounts[key] || 0) + 1;
            if (statName) {
                // 사진별 세트 선택 전 전역 풀 제외
                // 모든 사진 확인 시 선택 세트 포함 스탯 풀 재구성
    ...
    for (const [key, count] of Object.entries(pieceCounts)) {
        const [pieceKey, uniqueShardName = ''] = key.split('|||');
        // 마지막 '-'를 기준으로 조각 이름과 등급 분리
        const lastDashIndex = pieceKey.lastIndexOf('-');
        const pieceName = pieceKey.substring(0, lastDashIndex);
        const grade = pieceKey.substring(lastDashIndex + 1);
        result.push({
            pieceName: pieceName,
            grade: grade,
            count: count,
...
function detectPieceBoxes(src, gray, img) {
    ...
    // 고정 명도 검출 실패 대체 경로
    // 반복되는 컬러 카드 사각형을 이용해 누락된 박스를 보완
    const colorBoxes = csDetectColoredCardBoxes(src);
    // 최신 인벤토리 화면은 카드 색상이 반복되는 구조라 컬러 카드 검출이 더 안정적
    // 명도 윤곽선 내부 밝은 요소 오인 방지
    // 반복 카드가 2개 이상 잡히면 컬러 카드 좌표를 우선 사용
    ...
    // 두 번째 줄과 첫 번째 줄의 간격이 화면 높이에 비해 작을 때 한 줄로 오인
    // 카드 높이 기준 행 묶음·상하좌우 정렬
    if (boxes.length > 0) {
...
function csEstimateCardBackground(src, box) {
    // 카드 테두리 바로 옆은 어두운 외곽선이라 등급색으로 사용할 수 없음
    // 조각이 거의 닿지 않는 카드 안쪽 양옆/하단을 여러 곳 샘플링한 뒤,
    // 가장 많이 모이는 색상 군집의 중앙값을 카드 배경색으로 사용
    const points = [
        [0.12, 0.28], [0.88, 0.28],
        [0.10, 0.50], [0.90, 0.50],
        [0.12, 0.74], [0.88, 0.74],
...
function detectGreenTagOffsetFromMat(mat) {
    ...
    for (let y = 0; y < maxScanHeight; y++) {
        ...
        if (greenXs.length > 0 && mat.cols > 0) {
            ...
            // 장착중 태그는 넓고 연속된 초록 띠다. 조각 모서리의 작은 초록 장식은 제외
...
function extractUniqueShapeFromRoiWithDebug(iconRoi, bgColor, index, hasGreenTag = false) {
    ...
    if (totalFilled === 0) {
        return { shape: [], binary, gridInfo: '유니크 그리드: 0x0\n', dots: [], gridSizeX: 0, gridSizeY: 0, matchScore: null, pieceBounds: null };
    }
    ...
    if (!match || match.score < 0.01) {
        return { shape: [], binary, gridInfo: `유니크 매칭 실패\n총 픽셀: ${totalFilled}\n`, dots: [], gridSizeX: 0, gridSizeY: 0, matchScore: match ? match.score : null, pieceBounds: bbox };
    }
    ...
    return { shape: match.shape, binary, gridInfo, dots, gridSizeX: match.cols, gridSizeY: match.rows, matchScore: match.score, pieceBounds: bbox };
}

function cookieSimExtractUniqueCellFeatures(iconRoi, shape, pieceBounds) {
    if (!iconRoi || !Array.isArray(shape) || shape.length === 0 || !pieceBounds) return [];
    ...
    sortedShape.forEach(([row, col]) => {
        const cellX0 = pieceBounds.x + col * pieceBounds.width / cols;
        const cellX1 = pieceBounds.x + (col + 1) * pieceBounds.width / cols;
        const cellY0 = pieceBounds.y + row * pieceBounds.height / rows;
        const cellY1 = pieceBounds.y + (row + 1) * pieceBounds.height / rows;
        const padX = Math.max(1, Math.floor((cellX1 - cellX0) * 0.08));
        const padY = Math.max(1, Math.floor((cellY1 - cellY0) * 0.08));
        const x0 = Math.max(0, Math.floor(cellX0) + padX);
        const x1 = Math.min(iconRoi.cols, Math.ceil(cellX1) - padX);
        const y0 = Math.max(0, Math.floor(cellY0) + padY);
        const y1 = Math.min(iconRoi.rows, Math.ceil(cellY1) - padY);
        ...
        for (let y = y0; y < y1; y += 2) {
            for (let x = x0; x < x1; x += 2) {
                ...
                const isGreenTag = g > 145 && g > r + 30 && g > b + 30;
                if (isWhite || isGreenTag) continue;
                red += r;
                green += g;
                blue += b;
                count++;
        ...
        features.push(red / total, green / total, blue / total, (red + green + blue) / (3 * 255));
    });
...
function extractShapeFromImageWithDebug(src, box, bgColor, index, grade) {
    ...
    // 일반·유니크 카드 내부 전체 분석
    // 유니크 장착중 태그 영역 선택 제거
    const analysisRoi = fullIconRoi;
    const ownsAnalysisRoi = false;
    ...
    const { shape, binary, gridInfo, dots, gridSizeX, gridSizeY, matchScore, pieceBounds } = extraction;
    const uniqueVisualFeature = useUniqueExtractor
        ? cookieSimExtractUniqueCellFeatures(analysisRoi, shape, pieceBounds)
        : [];
    const uniqueArtFeature = useUniqueExtractor
        ? cookieSimExtractUniqueArtFeature(analysisRoi)
        : [];
    const uniqueSpatialFeature = useUniqueExtractor
        ? cookieSimExtractUniqueSpatialFeature(analysisRoi)
        : [];
    const uniqueCardFeature = useUniqueExtractor
        ? cookieSimExtractUniqueCardFeature(analysisRoi)
        : [];
...
function csBuildCommonPieceMask(iconRoi, bgColor, hasGreenTag = false) {
    ...
    // 카드 전체 크기의 사각형으로 잡힌다. 테두리 영역을 분석 대상에서 제외
    ...
    for (let y = topLimit; y < bottomLimit; y++) {
        for (let x = sideMargin; x < iconW - sideMargin; x++) {
            ...
            // 초록·연두 조각 장착중 태그 오인 방지
            // 태그가 실제로 감지된 카드에서, 잘라낸 뒤 상단에 남은 초록 픽셀만 제외
...
function extractShapeFromRoiWithDebug(iconRoi, bgColor, index, hasGreenTag = false, greenOffset = 0) {
    // 카드와 조각의 색 조합에 영향을 덜 받는 짙은 외곽선 판별을 우선
    ...
    // 외곽선이 지나치게 흐린 구버전 캡처는 카드 배경색 대비 방식으로 다시 확인
...
function extractShapeFromRoiLegacyWithDebug(iconRoi, bgColor, index, hasGreenTag = false) {
    ...
    const estimatedCellSize = minDim / Math.max(1, Math.floor(minDim / 20)); // 칸당 약 20픽셀
...
function findPieceNameByShape(extractedShape) {
    ...
    function csOpenRecognizedSetPicker(currentSet, onPick) {
        ...
        btn.addEventListener('pointerdown', (event) => {
            // 모바일에서 포인터 다운에 바로 열면 이어지는 클릭 이벤트로
            event.stopPropagation();
        });
        btn.addEventListener('click', toggleMenu);
    ...
    function renderRecognizedPieceCards(pieceData) {
        ...
        if (!Array.isArray(pieceData) || !pieceData.length) {
            // 배치 결과 카드처럼 업로드 전에도 섹션을 보여주고 안내 문구를 표시
        ...
        visibleData.forEach((data) => {
            ...
            if (grade !== 'unique') {
                ...
                normalStatChip = cookieSimMakeStatChip(null);
                const recognizedUniqueCandidates = cookieSimUniqueNamesForPiece(data.basePieceName);
                const resolvedRecognizedUniqueName = cookieSimSetUniqueShardNameChip(
                    normalStatChip,
                    data.uniqueShardName,
                    recognizedUniqueCandidates,
                    (nextName) => {
                        data.uniqueShardName = nextName;
                        pieceBlock.dataset.uniqueShardName = nextName;
                        fillPiecesFromCV(csRecognizedResults);
                ...
                data.uniqueShardName = resolvedRecognizedUniqueName;
                pieceBlock.dataset.uniqueShardName = resolvedRecognizedUniqueName;
            }
            ...
            normalStatChip.style.setProperty('padding', '4px 6px', 'important');
            normalStatChip.style.setProperty('display', 'block', 'important');
            normalStatChip.style.setProperty('min-width', '0', 'important');
            normalStatChip.style.setProperty('font-size', '10px', 'important');
            normalStatChip.style.setProperty('font-weight', '800', 'important');
            if (grade === 'unique') {
                normalStatChip.style.setProperty('padding', '1px 6px', 'important');
                normalStatChip.style.setProperty('line-height', '20px', 'important');
            }
            pieceBlock.appendChild(normalStatChip);
            cookieSimApplyOverflowAlignment(normalStatChip);
            ...
            preview.addEventListener('click', (ev) => {
                ...
                openCookieSimPiecePicker(data.basePieceName, (newPieceName) => {
                    ...
                    if (grade === 'unique') {
                        data.uniqueShardName = cookieSimResolveUniqueShardName(
                            cookieSimUniqueNamesForPiece(newPieceName),
                            data.uniqueShardName
                        );
                    }
                    csRecognizedSyncAndRender();
        ...
        addPieceCard.addEventListener('click', () => {
            csOpenRecognizedGradeSelectModal((gradeKey) => {
                openCookieSimPiecePicker(null, (pName) => {
                    ...
                    const uniqueShardName = gradeKey === 'unique'
                        ? cookieSimResolveUniqueShardName(cookieSimUniqueNamesForPiece(pName), null)
                        : null;
    ...
    function solve() {
        ...
        const recognizedUniqueInventory = cookieSimBuildRecognizedUniqueInventory();

        Object.entries(PIECES).forEach(([name, piece]) => {
            if (piece.isUnique) {
                // 유니크 등급별 입력 합계·이미지 인식 이름 연결
                const uniqueScore = 2000;
                const gradeInputs = ['rare', 'epic', 'super']
                    .map(grade => document.getElementById(`piece-count-${name}-${grade}`))
                    .filter(Boolean);
                let count = gradeInputs.reduce(
                    (sum, input) => sum + Math.max(0, parseInt(input.value || '0', 10) || 0),
                    0
                );
                if (count <= 0) {
                    const uniqueInput = document.getElementById(`piece-count-${name}-unique`);
                ...
                const recognizedNames = recognizedUniqueInventory[name] || [];
                for (let i = 0; i < count; i++) {
                    const uniqueShardName = recognizedNames[i] || null;
                    ...
                    piecesToUse.push({
                        ...
                        grade: 'unique',
                        uniqueShardName,
                        uniquePriorityRank: cookieSimUniquePriorityRank(uniqueShardName)
                    });
        ...
        // 현재 쿠키의 유니크 우선순위에 포함되지 않은 조각은 배치 후보에서 제외
        // 보유 유니크가 있어도 우선순위에 맞는 조각이 하나도 없으면 일반 조각만으로 탐색
        const uniqueCandidateCountBeforePriority = piecesToUse.filter(piece => piece.isUnique).length;
        piecesToUse = piecesToUse.filter(piece =>
            !piece.isUnique || Number(piece.uniquePriorityRank || 1000) < 1000
        );

        // 유니크 최고 우선순위 종류만 배치 후보
        const recognizedUniqueRanks = piecesToUse
            .filter(piece => piece.isUnique)
            .map(piece => Number(piece.uniquePriorityRank || 1000));
        if (recognizedUniqueRanks.length > 0) {
            const bestAvailableUniqueRank = Math.min(...recognizedUniqueRanks);
            piecesToUse = piecesToUse.filter(piece =>
                !piece.isUnique || Number(piece.uniquePriorityRank || 1000) === bestAvailableUniqueRank
            );
        }
        piecesCellCount = piecesToUse.reduce((sum, piece) => sum + piece.shape.length, 0);

        if (piecesToUse.length === 0) {
            const excludedUnique = uniqueCandidateCountBeforePriority > 0;
            const en = (typeof window !== 'undefined' && window.COOKIE_SIM_LANG === 'en');
            solutionSummary.textContent = excludedUnique
                ? (en
                    ? ' No unique shard matches this cookie priority. Add regular shards first!'
                    : ' 현재 쿠키의 우선순위에 맞는 유니크 조각이 없습니다. 일반 조각을 먼저 입력해주세요!')
                : (en ? ' Add shards first!' : ' 조각을 먼저 입력해주세요!');
            return;
        }
        ...
        if (PRIORITIZE_HIGH_SCORE) {
            piecesToUse.sort((a, b) => {
                const getPriorityLevel = (set) => {
                    ...
                    return index === -1 ? 4 : index + 1; // 우선순위 값
                };
                ...
                if (a.isUnique && b.isUnique) {
                    const rankDiff = (a.uniquePriorityRank || 1000) - (b.uniquePriorityRank || 1000);
        ...
        setTimeout(async () => {
            ...
            allSolutions.forEach(sol => {
                ...
                sol.processed = processed; // 후속 사용값 저장
            });
            ...
            allSolutions.sort((a, b) => {
                ...
                const aUniqueRank = Number(a.uniquePriorityRank || 1000);
                const bUniqueRank = Number(b.uniquePriorityRank || 1000);
                if (aUniqueRank !== bUniqueRank) return aUniqueRank - bUniqueRank;
    ...
    function processDlxSolution(solution, score) {
        ...
        let targetCellCount = 0; // 초기 배치 가능 칸 수
        gridState.forEach((unlocked, i) => {
            if (unlocked) {
                newBoard[i] = 0; // 배치 가능 칸 초기화
                targetCellCount++;
            }
        });
        ...
        const pieceGrades = {}; // 조각별 등급
        const pieceSets = {}; // 조각별 세트
        const setCellCounts = {}; // 세트별 칸 수
        ...
        solution.forEach(node => {
            ...
            if (pieceNode.pieceInfo) {
                ...
                pieceGrades[currentPieceId] = piece.grade || 'rare'; // 등급 저장

                // 세트 정보 추출 및 카운트 (유니크 조각은 세트 보너스에 포함하지 않음)
                const pieceSet = piece.set || null;
                pieceSets[currentPieceId] = pieceSet; // 세트 저장
    ...
    function renderSolution(board, totalScore = 0, solutionNumber = 1, usedPieces = [], pieceGrades = {}, pieceSets = {}, setBonusDetails = {}, baseScore = 0, setBonus = 0) {
        ...
        if (csGlassStats.length > 0) {
            ...
            (function assignPiecesToStats() {
                ...
                regularPieces.forEach(({ id, size }) => {
                    ...
                    if (picked) {
                        ...
                        // 목표 스탯이 아니거나 스탯 인식 실패한 조각은 회색으로 표시
            ...
            let csCurrentBoardView = 0; // 0: 세트 효과, 1: 인식 스탯
    ...
    function createDlxMatrix(board, pieces) {
        ...
        root.L = currentHeader;

        for (let i = 0; i < pieces.length; i++) {
            const piece = pieces[i];
            const pieceSecondaryCol = colMap.get(`piece_${piece.name}`); // 조각 인스턴스 보조 열
    ...
    let bestUniquePriorityRank = 1000;
    let bestSolution = [];
    let bestCellsFilled = 0;
    let allSolutions = [];
    let maxUniquePieces = 1;
    let maxRegularPieces = 15;
    let requireUnique = false; // 2차 탐색 유니크 포함 결과
    let dlxTimeBudget = MAX_TIME_MS; // 탐색 단계별 시간 예산
    let dlxNodesVisited = 0; // 탐색 노드 수
    let dlxLastYield = 0; // 마지막 이벤트 루프 양보 시각
    ...
    async function search(root, partialSolution = [], currentScore = 0, prioritySets = []) {
        ...
        const setCellCounts = {}; // 현재 부분 해 세트별 칸 수 계산
        let partialHasUnique = false;
        let currentUniquePriorityRank = 1000;
        partialSolution.forEach(node => {
            ...
            if (pieceNode.pieceInfo) {
                const { piece, pos } = pieceNode.pieceInfo;
                if (piece.isUnique) {
                    partialHasUnique = true;
                    currentUniquePriorityRank = Math.min(
                        currentUniquePriorityRank,
                        Number(piece.uniquePriorityRank || 1000)
                    );
                }
        ...
        // 배치 순위: 목표 세트·유니크 우선순위·총 저항·채운 칸
        ...
        if (recordable && (currentTargetsMet > bestTargetsMet ||
            (currentTargetsMet === bestTargetsMet && (currentUniquePriorityRank < bestUniquePriorityRank ||
             (currentUniquePriorityRank === bestUniquePriorityRank && (currentTotalResistance > bestTotalResistance ||
        ...
        if (recordable && (uncoveredPrimaryCount === 0 || partialSolution.length > 0)) {
            ...
            const solutionData = {
                ...
                uniquePriorityRank: currentUniquePriorityRank,
                totalResistance: currentTotalResistance
            };
            if (duplicateIndex < 0) {
                allSolutions.push(solutionData);
            } else if (currentUniquePriorityRank < Number(allSolutions[duplicateIndex].uniquePriorityRank || 1000)) {
                allSolutions[duplicateIndex] = solutionData;
            }
            // 목표 달성 → 유니크 우선순위 → 총 저항 → 칸 수 순으로 정렬
            allSolutions.sort((a, b) =>
                (b.targetsMet || 0) - (a.targetsMet || 0) ||
                Number(a.uniquePriorityRank || 1000) - Number(b.uniquePriorityRank || 1000) ||
        ...
        if (PRIORITIZE_HIGH_SCORE) {
            ...
            rowsToExplore.sort((a, b) => {
                ...
                if (pieceA.isUnique && pieceB.isUnique) {
                    const rankDiff = Number(pieceA.uniquePriorityRank || 1000) - Number(pieceB.uniquePriorityRank || 1000);
                    if (rankDiff !== 0) return rankDiff;
                }
                ...
                if (isSetAMaxed && !isSetBMaxed) return 1; // 21칸 미달 세트 우선
                if (!isSetAMaxed && isSetBMaxed) return -1; // 21칸 미달 세트 우선
        ...
        for (const { rowNode: r } of rowsToExplore) {
            ...
            if (pieceNode.pieceInfo) {
                ...
                if (actuallyNewCells.length > 0) {
                    ...
                    const potentialUniqueRank = piece.isUnique
                        ? Math.min(currentUniquePriorityRank, Number(piece.uniquePriorityRank || 1000))
                        : currentUniquePriorityRank;
                    const shouldExplore =
                        (piece.isUnique && uniqueCount === 0) ||
                        reachableTargets > bestTargetsMet ||
                        (reachableTargets === bestTargetsMet && potentialUniqueRank < bestUniquePriorityRank) ||
                        (reachableTargets === bestTargetsMet &&
