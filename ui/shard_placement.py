...
def _unique_ingame_card_feature(path: Path) -> list[float]:
    """Create a fixed-position thumbnail fingerprint used by the browser.

    The previous recognizer cropped the foreground to its own bounding box. In the
    browser that crop changes whenever the green ``장착중`` tag overlaps the shard,
    so the runtime feature did not match the stored sample even though the exact
    screenshot had been added. This feature deliberately keeps the artwork at its
    original position inside the yellow card and only removes the gold field and
    green tag. A small translation-tolerant distance is applied in JavaScript.
    """
    if not path.exists():
        return []
    ...
    with Image.open(path) as source:
        ...
        right = max(left + 1, round(width * 0.94))
        bottom = max(top + 1, round(height * 0.94))
