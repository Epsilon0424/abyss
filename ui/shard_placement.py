...
def _unique_reference_cell_features(path: Path, shape: list[list[int]]) -> list[float]:
    """Build per-cell color fingerprints used by the browser image recognizer."""
...
def _unique_reference_spatial_feature(path: Path) -> list[float]:
    """Build a compact spatial RGBA fingerprint for unique artwork.

    Shape extraction can occasionally flatten the staggered Road Nightmare shard
    into a plain 2x4 rectangle when the equipped tag overlaps its upper edge.
    The spatial fingerprint keeps the artwork layout, allowing the browser to
    distinguish it from the two rectangular dream shards.
    """
    if not path.exists():
        return []
...
def _unique_reference_art_feature(path: Path) -> list[float]:
    """Build a global HSV/RGB fingerprint for unique artwork recognition.

    The in-game thumbnail can occasionally lose one of the white cut-out corners,
    causing a staggered unique shard to be read as a plain rectangle.  This
    fingerprint compares the artwork itself so the name can still be recovered.
    """
    if not path.exists():
        return []
...
def _unique_ingame_art_feature(path: Path) -> list[float]:
    """Build the same artwork fingerprint used by OpenCV.js for game thumbnails.

    The clean source artwork and the small yellow in-game inventory thumbnail have
    very different color distributions.  Real thumbnail samples are therefore kept
    separately and compared before the silhouette fallback is used.
    """
    if not path.exists():
        return []
...
def _unique_ingame_spatial_feature(path: Path) -> list[float]:
    """Build the same normalized foreground fingerprint as OpenCV.js.

    Histogram-only comparison loses the artwork layout, so visually unrelated
    unique shards with similar purple/blue palettes can receive the same name.
    This feature removes the gold card and equipped tag, crops the remaining
    artwork, and stores its normalized 20x20 RGBA layout.
    """
    if not path.exists():
        return []
    ...
    # Equivalent to a small 2x2 MORPH_OPEN without adding an OpenCV dependency.
    ...
    # Keep connected foreground components that are not card-edge remnants.
...
def _unique_ingame_card_feature(path: Path) -> list[float]:
    """Create the fixed-position card fingerprint used by the browser.

    Files in ``img/유니크조각_인게임`` are already the card's 6% inner area,
    matching ``extractShapeFromImageWithDebug()`` in JavaScript. Cropping those
    samples by another 6% changes the artwork scale and position, so the browser
    feature and stored sample no longer use the same coordinate system.
    """
    if not path.exists():
        return []
    ...
    for y in range(_UNIQUE_CARD_FEATURE_SIZE):
        for x in range(_UNIQUE_CARD_FEATURE_SIZE):
            ...
            # White letters inside the equipped tag survive the green mask. Remove
            # only those bright pixels in the top band; pale artwork elsewhere stays.
            is_tag_text = (
...
def _unique_ingame_sample_image_uris(unique_name: str) -> list[str]:
    """Return embedded PNG samples for browser-side OpenCV.js feature generation."""
    sample_dir = _UNIQUE_INGAME_REFERENCE_DIR / unique_name
    if not sample_dir.exists():
        return []
...
def _build_unique_reference_metadata() -> dict[str, dict]:
    """Build reference metadata only as a fallback when the frozen file is missing."""
...
def _unique_reference_metadata() -> dict[str, dict]:
    """Load one identical metadata snapshot in local and Streamlit environments.

    Pillow/NumPy versions previously changed the sample feature values at runtime.
    The frozen metadata keeps the fallback values identical, while the browser later
    regenerates all in-game sample features with the same OpenCV.js functions used
    for the uploaded screenshot.
    """
...
def _unique_priority_for_cookie(cookie_name: str) -> list[str]:
    """Return the recognized unique-shard preference order for the active cookie."""
...
def _patch_solver_js(js: str, english: bool = False) -> str:
    ...
    # 유니크 입력 합산과 인식 우선순위 처리는 static/shard_placement.js에서 직접 수행합니다.
...
/* cookie-sim: redesigned shard controls — recognition first, one clear primary action */
.cs-upload-block {{
...
/* Bottom breathing room for the Auto Shard Placement card. */
...
/* cookie-sim: unify soft panel backgrounds with the Auto Recognition panel */
.cs-target,
.cs-result-card .cs-result-board-row .cs-board-panel,
.cs-result-card .cs-result-toggle-wide,
.cs-result-card .cs-result-toggle-button,
...
body.cs-theme-dark .cs-result-card .cs-result-static-chip,
body.cs-theme-dark .cs-result-card .cs-set-bonus-box,
body.cs-theme-dark .cs-result-card .cs-glass-carousel.cs-glass-single .cs-glass-pages > div:not([style*="display: none"]).cs-glass-single-page,
body.cs-theme-dark .cs-result-card .cs-glass-pages > div::before {{
    background: #292b2f !important;
...
@media (prefers-color-scheme: dark) {{
    ...
    body.cs-theme-system .cs-result-card .cs-glass-carousel.cs-glass-single .cs-glass-pages > div:not([style*="display: none"]).cs-glass-single-page,
    body.cs-theme-system .cs-result-card .cs-glass-pages > div::before {{
        background: #292b2f !important;
        background-color: #292b2f !important;
    }}
...
(function() {{
    ...
    function cookieSimNormalizePieceCard(card) {{
        ...
        if (isUniqueCard && statChip) {{
            ...
            // 조각 스탯 수정과 같은 전체 모달을 열어 선택하도록 유지한다.
        ...
        // 유니크 이름은 한 줄로 유지하고 뒤쪽만 말줄임 처리한다.
        // 스탯 카드 높이는 라이트·다크 모드 모두 다크 모드 기준인 156px로 통일한다.
    ...
    function tuneModal(root) {{
        ...
        document.querySelectorAll('body > div[style*="z-index: 2000"]').forEach(modal => {{
            ...
            modal.querySelectorAll('div[style*="flex-wrap"]').forEach(list => {{
                ...
                list.style.padding = '0 2px 12px 0';
                list.style.scrollPaddingBottom = '12px';
