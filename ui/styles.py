...
@media (max-width: 760px){
  /* 모바일에서는 바깥 여백뿐 아니라 중첩된 카드의 좌우 패딩도 함께 줄여
     SELECT / RESULT / Notes의 실제 콘텐츠 폭을 넓힌다. */
  section.main > div.block-container,
  [data-testid="stMainBlockContainer"],
  [data-testid="stAppViewContainer"] .block-container{
  ...
  /* Streamlit 1.48에서는 열 testid가 stColumn이다.
     기존 column 선택자만 사용하면 모바일에서 0.75:2.45 열 비율의
     남은 폭이 오른쪽 여백처럼 유지되므로 두 선택자를 모두 지원한다. */
  ...
  /* :has()의 열 선택이 제한되는 일부 모바일 WebView에서도
     메인 2열의 직접 자식 폭을 확실히 해제한다. */
  .st-key-outer_shell div[data-testid="stHorizontalBlock"]:has(.st-key-panel_select):has(.st-key-panel_result)
...
.st-key-party_group_jackfruit:has([class*="party_slot2"])
  div[data-testid="stElementContainer"]:has([class*="party_slot1"]),
...
.st-key-party_group_jackfruit:has([class*="party_slot2"]) [class*="party_slot1"],
...
.st-key-party_group:not(:has([class*="party_slot2"]))
  div[data-testid="stElementContainer"]:has([class*="party_slot1"]),
.st-key-party_group_jackfruit:not(:has([class*="party_slot2"]))
  div[data-testid="stElementContainer"]:has([class*="party_slot1"]),
...
.st-key-party_group_jackfruit:not(:has([class*="party_slot2"])) [class*="party_slot1"],
...
.st-key-party_group_jackfruit div[data-testid="stElementContainer"]:has([class*="party_slot2"]),
...
/* 잠재력 자동/수동 드롭다운:
   공통 .ctl-label의 글꼴/크기는 그대로 쓰되, 세부사항 최상단의 중첩 컨테이너에서는
   공통 음수 하단 margin(-10px)이 드롭다운을 위로 끌어올리고 제목 margin이 위로 붕괴할 수 있다.
   그래서 잠재력 블록만 '보이는 간격'을 기본/Setting과 맞추고 margin 붕괴는 막는다. */
...
/* 잠재력 제목은 쿠키명 / Hidden Power를 각각 한 덩어리로 두고,
   폭이 부족하면 Hidden Power 전체가 둘째 줄로 내려간다. */
[class*="st-key-potential_mode_block_"]{
...
[class*="st-key-potential_mode_block_"] .potential-title-cookie,
[class*="st-key-potential_mode_block_"] .potential-title-name{
...
/* 영어에서는 쿠키명과 Hidden Power를 항상 두 줄로 분리해 좁은 패널에서도 넘치지 않게 한다. */
[class*="st-key-potential_mode_block_"] .potential-title-en{
  display: grid !important;
  grid-template-columns: minmax(0, 1fr) !important;
  row-gap: 1px !important;
}
[class*="st-key-potential_mode_block_"] .potential-title-en .potential-title-cookie,
[class*="st-key-potential_mode_block_"] .potential-title-en .potential-title-name{
  width: 100% !important;
  max-width: 100% !important;
  min-width: 0 !important;
}
...
/* 패널 폭이 좁아지면 Hidden Power 전체를 둘째 줄로 내린다. */
...
/* 수동 잠재력 요약 상자 = 팝업 열기 버튼.
   자동/수동 드롭다운과 한 덩어리로 보이되 위쪽은 살짝 띄운다. */
...
/* 잠재력 수동 설정 모달: 전용 다이얼로그 프레임
   - 제목 텍스트는 숨기고 X만 유지
   - 흰색 다이얼로그 폭/안쪽 패딩 축소
   - 모바일에서도 입력칸은 항상 2열 유지 */
...
/* Streamlit 다이얼로그 헤더의 빈 제목은 완전히 흐름에서 빼고 X만 우상단에 겹쳐 둔다.
   제목을 없앤 만큼 첫 안내문이 바로 위에서 시작하도록 한다. */
...
/* 다이얼로그 본문은 제목 자리까지 끌어올리고, 흰 박스와 내용 사이 패딩도 축소한다. */
...
/* 숫자 입력칸은 hover/focus/click 상태에서도 외형을 바꾸지 않는다.
   Streamlit/BaseWeb의 accent 포커스 링까지 전부 무효화한다. */
...
/* =====================================================
   Potential dialog final compact overrides
   - remove the reserved empty title space
   - keep the number stepper as one seamless gray control
   - vertically center the Done button text
===================================================== */
div[role="dialog"]:has([class*="st-key-potential_dialog_body_"]) {
  padding-top: 0 !important;
  padding-bottom: 0 !important;
...
/* st.dialog keeps header/title space even when the title text is hidden.
   Pull only this dialog body upward and reclaim that reserved space. */
...
/* 안내 문구도 위쪽에서 바로 시작하도록 불필요한 기본 여백 제거 */
...
/* 숫자 입력 스테퍼: 바깥 회색 박스 하나만 보이게 하고 내부 분할선/hover/focus 외곽선을 제거 */
div[role="dialog"]:has([class*="st-key-potential_dialog_body_"]) div[data-testid="stNumberInput"] div[data-baseweb="input"],
div[role="dialog"]:has([class*="st-key-potential_dialog_body_"]) div[data-testid="stNumberInput"] div[data-baseweb="base-input"] {
...
/* 잠재력 수동 요약 박스 최종 보정.
   일반 secondary button 스타일보다 뒤에서 적용해 레퍼런스처럼 회색 박스로 고정한다. */
...
# Final mobile scrollbar geometry: keep the Streamlit page, BaseWeb dropdowns,
# and every nested scroll area at the same 4px width used by the shard panel.
_MOBILE_SCROLLBAR_GEOMETRY_CSS = r"""
<style>
/* Keep the page scrollbar thin, and target every element only inside the
   BaseWeb dropdown portal. The dropdown's actual scroll owner differs by
   browser, while the 0px horizontal height prevents the old PC-only bar. */
html,
body,
[data-testid="stApp"],
[data-testid="stAppViewContainer"],
...
/* BaseWeb dropdowns must scroll vertically only. This removes the desktop-only
   horizontal bar while retaining the 4px vertical scrollbar. */
div[data-baseweb="popover"] [role="listbox"],
...
# 잠재력 수동 설정은 팝업이 아니라 세부사항 탭 안에서 바로 편집한다.
# 좁은 패널에서도 잠재력 항목은 항상 한 줄에 2개씩 유지한다.
...
/* 잠재력 각 행을 실제 keyed container로 분리해 간격을 확실히 적용한다. */
...
/* 행 간격은 margin이 아니라 실제 높이를 가진 spacer로 고정한다. */
...
/* 화면 폭과 관계없이 항상 2열, 좌우 간격은 5px 고정. */
...
/* 상태 영역 전체는 오른쪽 열 폭 안에 고정하고, 텍스트만 말줄임한다.
   부모를 overflow:hidden으로 자르면 margin-top까지 포함한 높이가 잘려
   글씨의 아래쪽 절반이 사라질 수 있으므로 부모 클리핑은 하지 않는다. */
...
# 라이트모드의 #E5E7EB가 DARK_THEME_CSS에 같이 들어가므로,
# 다크 테마에서만 선택자를 뒤에서 다시 덮어쓴다.
