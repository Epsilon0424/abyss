...
/* SELECT 패널 하단: 마지막 선택칸과 구분선 사이 실제 보이는 간격을 약 5px로 맞춘다.
   hr 자체 여백뿐 아니라 Streamlit이 Markdown 요소 바깥에 만드는 기본 요소 간격도 함께 상쇄한다. */
...
===================================================== */
/* 기본 탭 파티 드롭다운은 모든 슬롯을 동일한 간격으로 표시한다.
   기존의 큰 음수 margin 보정은 화면 폭/슬롯 수에 따라 드롭다운이 겹칠 수 있어 제거한다.
   세부사항 탭의 연속 드롭다운과 같은 4px 간격을 사용한다. */
...
/* 파티(서폿/스트/딜러) 선택칸 사이 간격을 세부사항 탭의 연속 드롭다운과 동일하게 맞춘다.
   Streamlit의 기본 요소 여백이 약 14px 추가되므로 -10px로 상쇄해 실제 보이는 간격을 약 4px로 유지한다. */
...
/* 딜러 메인: 첫 번째 역할 파티와 두 번째 역할 파티 사이에만
   Streamlit 기본 여백 14px가 추가로 남으므로 두 번째 행을 위로 당긴다.
   두 번째 행 뒤의 추가 딜러 슬롯은 문서 흐름상 함께 올라가므로
   파티 3개 드롭다운 사이 간격이 모두 동일해진다. */
...
/* 추가 딜러 슬롯은 party_dealer_group 내부에 들어가면서
   공통 아이콘의 +14px 세로 보정이 한 번 더 적용되어 아이콘만 아래로 내려간다.
   드롭다운 위치/간격은 그대로 두고 딜러 슬롯 아이콘만 14px 위로 맞춘다. */
...
/* 잠재력 제목도 계산 보정 안내처럼 공간이 충분하면 한 줄,
   좁아지면 단어 단위로 자연스럽게 줄바꿈한다. */
...
/* 잠재력 스탯 아이콘: 드롭다운 아이콘과 같은 방식으로 입력칸 왼쪽에 고정한다. */
.st-key-detail_tab_body [class*="st-key-potential_statitem__"]{
  position: relative !important;
}
