...
잭프루트맛 쿠키: Jackfruit Cookie
스테인드누가맛 쿠키: Stained Nougat Cookie
...
보물의 방향 : Where the Compass Points
꿈결 같은 휴식 : Dreamlike Breaktime
...
_EXTRA_ENGLISH = {
    ...
    "자동": "Auto",
    "수동": "Manual",
    "잠재력 설정": "Hidden Power Settings",
    "잠재력 설정 방식": "Hidden Power setting mode",
    "설정값 없음": "No values set",
    "각 잠재력의 장착 개수를 설정해 주세요.": "Set the number of slots for each Hidden Power stat.",
    "완료": "Done",
    "잠재력 직접 설정": "Set Hidden Power manually",
    "잠재력 사용 칸": "Hidden Power slots used",
    "잠재력은 총 8칸을 맞춰야 합니다.": "Hidden Power must use exactly 8 slots.",
    "설정한 잠재력을 고정한 상태에서 설탕유리조각을 다시 최적화합니다.": "Sugarglass shards are re-optimized with the selected Hidden Power fixed.",
    "자동에서는 기존 방식대로 잠재력과 설탕유리조각을 함께 최적화합니다.": "Auto mode optimizes Hidden Power and Sugarglass shards together as before.",
    "언어": "Language",
    "없음": "None",
    "신비": "Mystic",
    "충전은 타이밍": "Well-timed Recharge",
    ...
    "외딴 별의 여정": "Journey of a Distant Star",
    "플럼나이트": "Plumite",
    "허브그린드:백마법사의 의지": "Herberyl: Will of the White Mage",
    ...
    "낙인": "Branded",
    # 영어 번역 파일 이름·보정·설명 행
    # 장비/시즈 이름 번역을 덮어쓰지 않도록 기본 이름을 마지막에 강제 보정
    "영원의 대마술사": "Eternal Magician",
    "전설의 유령해적": "Ghost Captain",
    "달콤한 설탕 깃털": "Sugar Feather",
    "미지의 방랑자": "Mysterious Wanderer",
    ...
    "파티 쿠키 딜 기여도": "Party Cookie Damage Contribution",
    "총합": "Total",
    "일부 파티 쿠키의 딜 기여도를 계산하지 못했습니다.": "Some party cookie damage contributions could not be calculated.",
    "딜": "Damage",
...
def _parse_translation_text(text: str) -> dict[str, str]:
    ...
    for i, line in enumerate(lines):
        ...
        # 왼쪽 영어 행 번역 키 제외
        # 고정 영어 번역 파일 결과 행
        # 다시 번역표에 넣으면 영어 문장이 한 번 더 번역되어 중복
        if not has_kr(left):
            continue
...
def _tr_one(text) -> str:
    ...
    # 설탕유리조각 값 문구는 일부 변환 상태에서도 조각 이름을 영어로 보정
    # 일반 스탯명 내부 조각명 오인 방지
    # 전체 번역 딕셔너리 확인이 끝난 뒤에만 부분 치환을 실행
